...
*
*  Modified by Mark Kaplan, Raymond Irving, Greg Matthews and Ryan Thrash:
...
*      Enhancements Mark Kaplan, Raymond Irving, Ryan Thrash, LePrince, mrruben5, lloyd_barrett
*      Original code by Alex
*
...
*      &datetype      - the date column to use in sorting and display (values can be `createdon`, `pub_date`, `editedon`)
...
$datetype = isset ($datetype) ? $datetype : 'createdon';
// sort by either createdon, pub_date, editedon
...
/* API Method (does not allow for alternative sorting)
$callby = ($showPublishedOnly)? 'getActiveChildren' : 'getAllChildren';
$resource = $modx->$callby($resourceparent, 'createdon', 'DESC', $fields='*');
*/

$tblContent = $modx->db->config['table_prefix'].'site_content';
$activeClause = $showPublishedOnly ? 'AND published = 1' : '';
$query = "SELECT id , type , contentType , pagetitle , longtitle , description , alias , published , IF(pub_date > 0, pub_date, createdon) as pub_date, unpub_date , parent , isfolder , introtext , content , richtext , template , menuindex , searchable , cacheable , createdby , createdon , editedby , IF(editedon > 0, editedon, createdon) as editedon, deleted , deletedon , deletedby , menutitle , donthit , haskeywords , hasmetatags , privateweb , privatemgr , content_dispo , hidemenu  FROM $tblContent WHERE parent = $resourceparent $activeClause ORDER BY $datetype DESC";
...
if ($nrposts > 0) {
	for ($x = 0; $x < $stop; $x ++) {
		...
		if (!$trunc) {
			...
			if ((strstr($resource[$x]['content'], $splitter)) && $truncsplit) {
				$summary = array ();
		...
		$modx->setPlaceholder('date', strftime($date, $resource[$x]['createdon']));
