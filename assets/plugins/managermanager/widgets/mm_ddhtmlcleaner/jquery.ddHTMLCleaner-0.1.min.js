...
var b=d,a=0,g,c,h;for(e=f.extend(!0,{},this.defaults,e||{});g=this.reg.tag.exec(d);){var k=g.index+g[1].length;if(0<f.trim(g[3]).length)for(;c=this.reg.attr.exec(g[3]);)if("style"==c[2])for(;h=this.reg.style.exec(c[3]);)-1==f.inArray(h[1],e.validStyles)&&(b=this.substr_replace(b,"",k+c.index+c[1].length+h.index-a,h[0].length),a+=h[0].length);else(-1!=f.inArray(c[2],e.validAttrsForAllTags)||e.validAttrs[g[2]]&&-1!=f.inArray(c[2],e.validAttrs[g[2]]))&&0!=f.trim(c[3]).length||(b=this.substr_replace(b,
"",k+c.index-a,c[0].length),a+=c[0].length)}return b}}})(jQuery);