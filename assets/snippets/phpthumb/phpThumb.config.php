...
$PHPTHUMB_CONFIG['cache_directory'] = dirname(__FILE__).'/../../cache/images/';                            // set the cache directory relative to the phpThumb() installation - assets/images/phpthumb_cache by default
...
$PHPTHUMB_CONFIG['cache_directory_depth'] = 2; // If this larger than zero, cache structure will be broken into a broad directory structure based on cache filename. For example "cache_src012345..." will be stored in "/0/01/012/0123/cache_src012345..." when (cache_directory_depth = 4)
...
//$PHPTHUMB_CONFIG['cache_maxsize'] = 10000 * 1024 * 1024; // delete least-recently-accessed cached thumbnails when more than [10MB] of cached files are present (value is maximum bytesize of all cached files)
...
$PHPTHUMB_CONFIG['cache_source_directory'] = $PHPTHUMB_CONFIG['cache_directory'].'source/';  // set the cache directory for unprocessed source images
...
$PHPTHUMB_CONFIG['error_image_width']           = 250;      // default width for error images
...
$PHPTHUMB_CONFIG['error_die_on_error']          = false;     // die with error message on any fatal error (recommended with standalone phpThumb.php)
...
//$PHPTHUMB_CONFIG['mysql_query'] = 'SELECT `picture` FROM `products` WHERE (`id` = \''.mysql_real_escape_string(@$_GET['id']).'\')';
...
$PHPTHUMB_CONFIG['high_security_password']   = 'fFf43$ZyzCLsTws9HQ835eBeiKmtReGafpYpMgd9Jx1W3koNS5fxoNLoHWUWZEBwI';     // required if 'high_security_enabled' is true, must be at least 5 characters long
