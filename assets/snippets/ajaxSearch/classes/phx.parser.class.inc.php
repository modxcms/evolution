...
class PHxParser {
  ...
  function Parse($template='') {
    ...
    if ($this->debug && $this->debugLog) {
      $modx->logEvent(0,1,$this->createEventLog(), $this->name.' '.$this->version);
      $this->debugLog = false;
    }
  ...
  function ParseValues($template='') {
    ...
    $this->LogPass();
    $this->Log("Template input: ".$template);
    ...
    if ( preg_match_all('~\[(\[|!)([^\[\]]*?)(!|\])\]~s',$template, $matches)) {
        ...
        // for each detected snippt
        for($i=0; $i<$count; $i++) {

          $isCached = (trim($matches[1][$i])=="!") ? false : true; // is this snippet cached?
          $snippet = $matches[2][$i]; // snippet call

          if (!$isCached) { // if not cached
            $this->swapSnippetCache = $modx->snippetCache; // store the real cache
            $modx->snippetCache = array(); // empty cache
            $this->Log("MODx Snippet -> uncached: ".$snippet);
          } else { // if cached
            $this->Log("MODx Snippet -> cached: ".$snippet);
          }
          ...
          // If not Cached
          if (!$isCached) $modx->snippetCache = $this->swapSnippetCache; // put original cache back
        }
