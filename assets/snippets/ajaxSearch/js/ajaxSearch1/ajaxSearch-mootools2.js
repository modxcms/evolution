/*
 * AjaxSearch 1.9.0 - package AjaxSearch 1 - Mootools 1.2.4
...
var _version='1.9.0';var _opacity=1;var _liveSearch=0;var _minChars=3;var _init='none';var _pagingType=1;function activateSearch(){for(ias=0;ias<asvar.length;ias++){var asv=eval('('+asvar[ias]+')');activateAsInstance(asv);}}
...
else s='';if(s==as['bxt'])s='';as['s']=s;if(as['si']&&(s.length!=0)&&as['lvs']&&(s.length<as['mch']))return false;if((s.length==0)&&(as['ini']=='none'))return false;as['is']=true;if(!as['lvs'])as['ss'].disabled=true;var rbl=new Array("allwords","exactphrase","nowords");adv="oneword";for(var x=0;x<3;x++){if(rb=$(as['px']+'radio_'+rbl[x])){if(rb.checked==true)adv=rb.value;}}
