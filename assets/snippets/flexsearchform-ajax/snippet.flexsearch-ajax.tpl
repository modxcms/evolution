...
/* Update: 03/20/06
   Added parameters for ajaxsearch, and output them in a javascript block.
   This removed the need to set the variables in the ajaxsearch.php file.
*/

/* Modified By: KyleJ (kjaebker@muddydogpaws.com)
   Modified Date: 03/14/06
   Changes Made:
        1. Put code for doing the search and cleaning the string into a seperate file.
        2. Added code to allow for ajax view of results - see readme file for how to use
            ExampleCall - [!FlexSearchForm-ajax?ajaxSearch=1!]
            In the example I named this snippet FlexSearchForm-ajax
        3. For ajax css use the following
            id="FSF_ajax_searchResults"
            class="FSF_ajax_result"
            class="FSF_ajax_resultLink"
            class="FSF_ajax_resultDescription"
            class="FSF_ajax_more"
        4. The ajax functionality requires that you create a folder named
             flexsearchform-ajax in the snippet folder
        5. Added ids to the searchform
...
// CONFIGURE
  ...
  // --------------------------
   ...
   $ajaxSearch = (isset($ajaxSearch))? $ajaxSearch : 0;
...
include_once $snipPath."flexsearchform-ajax/FlexSearchForm.inc.php";
...
if (($validSearch && ($showSearchWithResults)) || $showSearchWithResults){
  $SearchForm .= '<form '.$searchFormId.' class="FSF_form" action="'.$searchAction.'" method="post">';
  ...
  $SearchForm .= '<input class="FSF_input" type="text" name="search" id="search" value="'.$searchBoxVal.'"  ';
...
if ($showResults) {
  if($validSearch) {
    ...
    $introMessage = '<p class="FSF_intro">'.$introMessage.'</p>';
...
if ($ajaxSearch) {
    $introMessage = '<div id="FSF_ajax_searchResults"></div>';
    $SearchForm .= $introMessage;
}
