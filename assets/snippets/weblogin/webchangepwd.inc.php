...
if(!$isPostBack && isset($_SESSION['validated'])){
	...
	if($ds['password']==md5($oldpassword)) {
		...
		if ($passwordgenmethod=="spec" && $_POST['specifiedpassword']!=$_POST['confirmpassword']) {
			$output = jsAlert("Password typed is mismatched",1).$tpl;
			return;
		}
		...
		if($specifiedpassword!="" && $passwordgenmethod=="spec") {
			...
			$output = jsAlert("No password generation method specified!").$tpl;
			return;
		}
		...
		if($rt!==true) {
			$output = jsAlert("An error occured while saving new password: $rt");
			return;
		}		
		...
		$output = jsAlert("Incorrect password. Please try again.").$tpl;
		return;
	}
}
