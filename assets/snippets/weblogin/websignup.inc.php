...
$declare = extractDeclarations($tpl);
...
if(!$isPostBack){
	...
	if($username=="") {
		$output = jsAlert("Missing username. Please enter a user name.").$tpl;
		return;
	}
	else {
		$sql = "SELECT id FROM ".$modx->getFullTableName("web_users")." WHERE username='$username'";
		if(!$rs = mysql_query($sql)){
			$output = jsAlert("An error occured while attempting to retreive all users with username $username.").$tpl;
			return;
		} 
		$limit = mysql_num_rows($rs);
		if($limit>0) {
			$output = jsAlert("Username is already in use!").$tpl;
			return;
		}		
	}
	...
	if($email=='' || !ereg("^[-!#$%&'*+./0-9=?A-Z^_`a-z{|}~]+", $email)){
		$output = jsAlert("E-mail address doesn't seem to be valid!").$tpl;
		return;
	}
	// verify password
	if ($_POST['password']!=$_POST['confirmpassword']) {
		$output = jsAlert("Password typed is mismatched").$tpl;
		return;
	}
	...
	if(!$rs = mysql_query($sql)){
		$output = jsAlert("An error occured while attempting to retreive all users with email $email.").$tpl;
		return;
	} 
	$limit = mysql_num_rows($rs);
	if($limit>0) {
		...
		if($row['id']!=$id) {
			$output = jsAlert("Email is already in use!").$tpl;
			return;
		}
	}
	...
	if (strlen($password) < 6 ) {
		$output = jsAlert("Password is too short!").$tpl;
		return;
	} 
	elseif($password=="") {
		$output = jsAlert("You didn't specify a password for this user!").$tpl;
		return;		
	}

	// verify form code
	if($_SESSION['veriword']!=$formcode) {
		$output = jsAlert("Incorrect form code. Please enter the correct code displayed by the image.").$tpl;
		return;
	}
	...
	$rs = mysql_query($sql);
	if(!$rs){
		$output = jsAlert("An error occured while attempting to save the user.").$tpl;
		return;
	} 		
	// now get the id
	$key=mysql_insert_id();
	...
	$rs = mysql_query($sql);
	if(!$rs){
		$output = jsAlert("An error occured while attempting to save the user's attributes.").$tpl;
		return;
	}
	...
	$rt = sendNewPassword($email,$username,$password,$fullname);
