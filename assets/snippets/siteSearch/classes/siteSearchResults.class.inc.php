...
class AjaxSearchResults {
    ...
    */
    function getSearchResults(&$msgErr) {
        global $modx;
        $results = array();
        include_once AS_PATH . "classes/siteSearchRequest.class.inc.php";
        ...
        if ($this->dbgRes) $this->asUtil->dbgRecord($this->asCfg->scfg, "SiteSearch - scfg");
        if ($this->dbgRes) $this->asUtil->dbgRecord($this->groupResults, "SiteSearch - group results");
        if ($this->dbgRes) $this->asUtil->dbgRecord($this->_groupMixedResults, "SiteSearch - group mixed results");
    ...
    function _getSubsiteParams($site, $subsite, &$msgErr) {
        ...
        if ($site != DEFAULT_SITE) {
            ...
            if (!function_exists($siteConfigFunction)) {
                $msgErr = '<br /><h3>SiteSearch error: search function ' . $siteConfigFunction .  ' not defined in the configuration file: ' . $this->asCfg->cfg['config'] . ' !</h3><br />';
                return false;
            }
            else {
                $sitecfg = $siteConfigFunction($site);
                if (!count($sitecfg)) {
                    $msgErr = '<br /><h3>SiteSearch error: Site ' .$site .  ' not defined in the configuration file: ' . $this->asCfg->cfg['config'] . ' !</h3><br />';
                    return false;
                }
            }
        }
        ...
        if ($subsite != DEFAULT_SUBSITE) {
            ...
            if (!function_exists($subsiteConfigFunction)) {
                $msgErr = '<br /><h3>SiteSearch error: search function ' . $subsiteConfigFunction .  ' not defined in the configuration file: ' . $this->asCfg->cfg['config'] . ' !</h3><br />';
                return false;
            }
            else {
                $subsitecfg = $subsiteConfigFunction($site,$subsite);
                if (!count($subsitecfg)) {
                    $msgErr = '<br /><h3>SiteSearch error: Subsite ' .$subsite .  ' of ' . $site . 'not defined in the configuration file: ' . $this->asCfg->cfg['config'] . ' !</h3><br />';
                    return false;
                }
            }
        }
            ...
            if (isset($this->asCfg->cfg['whereSearch'])) {
                ...
                foreach ($tables_array as $table) {
                    ...
                    if (($tbcode != 'content') && ($tbcode != 'tv') && ($tbcode != 'jot') && ($tbcode != 'maxigallery') && !function_exists($tbcode)) {
                        $msgErr = "<br /><h3>SiteSearch error: table $tbcode not defined in the configuration file: " . $this->asCfg->cfg['config'] . " !</h3><br />";
                        return false;
                    }
                }
            }
    ...
    */
    function _setSearchResults($site, $subsite, $rs) {
        ...
        if (($display == MIXED)) {
            ...
            if ($this->dbgRes) $this->asUtil->dbgRecord($this->_groupMixedResults[$ig], "SiteSearch - group mixed results");
            ...
            if ($this->asCfg->cfg['category']) {
                ...
                for ($i = 0;$i < $nbc;$i++) {
                    ...
                    if ($display == UNMIXED) {
                        ...
                        if ($this->dbgRes) $this->asUtil->dbgRecord($this->groupResults[$ig], "SiteSearch - group results");
                ...
                $row = array();
                if ($this->dbgRes) $this->asUtil->dbgRecord($rs, "SiteSearch - rs");
    ...
    */
    function _sortMixedResults() {
        if (isset($this->_groupMixedResults['results'])) {
            ...
            $this->nbGroups++;
            if ($this->dbgRes) $this->asUtil->dbgRecord($this->_groupMixedResults['results'], "SiteSearch - sorted noName results");
        }
    }
    /*
        ...
        if ($listTvs) {
            ...
            foreach ($tvs as $tv) {
                ...
                if (!$modx->db->getRecordCount($tplRS)) {
                    $msgErr = "<br /><h3>SiteSearch error: tv $tv not defined - Check your withTvs parameter !</h3><br />";
                    return false;
                }
            }
        }
