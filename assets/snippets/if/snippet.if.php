...
for ($i=1;$i<$total;$i++){
    ...
    switch (strtolower($operator)) {
        case '%'  :$output = ($subject %$operand==0) ? true : false; break;
        case '!=':
        case 'not':$output = ($subject != $operand)  ? true : false; break;
        case '<':
        case 'lt' :$output = ($subject < $operand)   ? true : false; break;
        case '>':
        case 'gt' :$output = ($subject > $operand)   ? true : false; break;
        case '<=':
        case 'lte':$output = ($subject <= $operand)  ? true : false; break;
        case '>=':
        case 'gte':$output = ($subject >= $operand)  ? true : false; break;
        case 'isempty':
        case 'empty':$output = empty($subject)       ? true : false; break;
    ...
    $eq=$output?$eq:false;
    $_ = explode(',','isempty,empty,!empty,notempty,not_empty,isnotempty,isnull,null,custom,==,=,eq,is');
    if(!in_array(strtolower($operator), $_)) $i++;
}
if ($lp==1){
    if ($left_part) {
        if (!empty($part_eq)){
