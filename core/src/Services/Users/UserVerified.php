...
class UserVerified implements ServiceInterface
{
    /**
     * @var \string[][]
     */
    ...
    public function process(): \Illuminate\Database\Eloquent\Model
    {
        ...
        $user = User::where('username', $this->userData['username'])
            ->where('verified_key', $this->userData['verified_key'])
            ->first();
        if (is_null($user)) {
