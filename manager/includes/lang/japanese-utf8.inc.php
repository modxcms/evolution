...
$_lang["access_permissions_docs_message"] = 'このリソースを所属させたいドキュメントグループを選択してください。アクセスを制限する必要がなければ「全ドキュメントグループ」にチェックを入れておきます。';
...
$_lang["access_permissions_no_resources_in_group"] = '無し';
...
$_lang["access_permissions_resources_tab"] = 'リソースグループの設定を一覧することができます。グループの作成・リネーム・削除もここで操作します。 また、ドキュメントがどのグループに属しているかを閲覧することもできます(リソース名を表示するには、idの上にマウスポインタを乗せてください)。 リソースをグループに追加したり、グループから削除するには、リソースを直接編集してください。';
...
$_lang["datepicker_offset_message"] = 'デートピッカー(カレンダー)で参照・選択できる過去年数';
...
$_lang["emailsubject_default"] = 'ログイン情報';
...
$_lang["export_site_success_skip_dir"] = 'このディレクトリをスキップ';
$_lang["export_site_success_skip_doc"] = 'このドキュメントをスキップ';
...
$_lang["import_site_failed_db_error"] = 'ドキュメントの複製中にデータベースエラーが発生しました: ';
...
$_lang["mail_check_timeperiod_title"] = 'メッセージチェックのタイミング:';
...
$_lang["publish_resource"] = 'リソースの公開';
$_lang["rb_base_dir_message"] = 'ファイルブラウザディレクトリ(アセットディレクトリ)への物理パスを入力してください。通常この設定は自動的に生成されますが、IISなど一部のサーバでは正常に検出されないことがあります。この場合は手入力で入力してください。';
...
$_lang["rb_webuser_message"] = '<a href="index.php?a=99">ウェブユーザー</a>にファイルブラウザの利用を許可するかどうかを選択します。 <strong>警告:</strong> ウェブユーザーにファイルブラウザの利用を許可するということは、管理画面ユーザーが利用するファイルを彼らに見せることになります。従って、信用できるウェブユーザーにのみ許可するべきでしょう。';
...
$_lang["resource_opt_folder_help"] = 'このリソースをコンテナとして明示的に扱いたい場合にチェックしてください。通常はサブドキュメントの有無により自動的に判定されます';
...
$_lang["resource_parent_help"] = '上のコンテナアイコンをクリックすると、このリソースの親(コンテナ)を選択できる状態になります。アイコンが点滅している状態で、親にしたいリソースをサイトツリー上でクリックしてください（アイコンの点滅は、再度アイコンをクリックすると元に戻ります）。';
...
$_lang["settings_strip_image_paths_message"] = 'ファイルブラウザで選択したファイルのsrc属性を相対パスで投稿画面に渡します。この設定を「いいえ」にした場合は、httpから始まるフルURLになります。投稿画面で利用するRTEによっては、受け取った値をさらに処理することがあります。';
$_lang["settings_strip_image_paths_title"] = '相対パスに変換する';
...
$_lang["tree_show_protected_message"] = '操作権限を持たないリソースも全てサイトツリーに表示します。「いいえ」に設定した場合は権限を持つリソースのみが表示されますが、親リソースに権限がない場合は、権限を持つサブリソースも表示されません。権限の設定は<a href="index.php?a=40">「管理画面へのアクセス許可」</a>で行ないます。';
...
$_lang["undelete_resource"] = 'リソースの復活';
$_lang["unpublish_date"] = '非公開日時';
$_lang["unpublish_events"] = '公開取消予定のリソース一覧';
$_lang["unpublish_resource"] = 'リソースの公開取消';
...
$_lang["use_alias_path_message"] = 'このオプションを「はい」に設定すると、リソースのエイリアスがパスとして表示されます。例えば、「child」というエイリアスを持ったリソース「parent」というエイリアスのリソースの下に配置されていると、エイリアスパスは「/parent/child.html」になります。<br /><strong>注意: このオプションを「はい」にするとき、つまりエイリアスパスを使用するときは、リソースから参照するものは全て（画像とかCSSとかJavaScriptなど）絶対パスで指定する必要があります。例えば、assets/imagesに対しては/assets/imagesと指定しなければなりません。baseタグを利用すると、この問題をスマートに解決できます。</strong>';
$_lang["use_alias_path_title"] = 'エイリアスパスを使用:';
$_lang["use_editor_message"] = 'リッチテキストエディターを使用しますか? HTMLを直接書いた方が良いのでしたら「いいえ」を選択してエディターを使わない設定にすることもできます。この設定は全てのリソースおよび全てのユーザーに適用されますので注意してください!';
