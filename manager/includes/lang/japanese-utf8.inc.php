 ...
 * Version:        2.01
 * MODx version:   0.9.5
 * Translated:     23 Nov. 2006 by eastbind<eastbind@bodenplatte.jp>
*/
...
$_lang["access_permissions_links_tab"] = 'ここでは、どのユーザグループがドキュメントグループにアクセス(例 編集、子ドキュメントの作成)できるかを指定します。ドキュメントグループをユーザグループに関連付けるには、ドロップダウンメニューよりドキュメントグループを選択して「グループの追加」をクリックしてください。ドキュメントグループとの関連付けを削除するには、「削除 ->」をクリックしてください。この作業を行うことにより、すぐに関連付けが解除されます。';
...
$_lang["access_permissions_users_tab"] = 'ここではユーザグループの設定を一覧することができます。グループの作成、リネーム、削除もここで操作します。また、ユーザがどのグループのメンバーになっているかを閲覧することができます。ユーザをグループに追加したり、グループから削除するには、ユーザを直接編集してください。管理者(権限ID 1が割り当てられているユーザ)は、常に全てのドキュメントにアクセスすることができます。ですから、管理者をグループに追加する必要はありません。';
...
$_lang["access_permission_denied"] = 'あなたには、このドキュメントにアクセスする権限がありません。';
$_lang["access_permission_parent_denied"] = 'あなたには、ここにドキュメントを作成する権限がありません!';
...
$_lang["alert_delete_self"] = 'あなた自身を削除する事はできません!';
$_lang["alias"] = 'エイリアス';
...
$_lang["blocked_minutes_message"] = 'ここでは上記の「ログイン失敗の回数」で指定した回数ログインに失敗したユーザのアカウントをロックする期間を分で指定します。必ず数字のみを入力してください。（カンマやスペースも駄目です。）';
...
$_lang["captcha_message"] = 'この設定を行うと、機械(およびscriptkiddyなハッキングスクリプト)で読み取ることのできないコードをユーザに入力させることで、セキュリティを強化することができます。';
...
$_lang["category_msg"] = 'このページではカテゴリでグループ化したリソースを一覧/編集できます。';
...
$_lang["charset_message"] = 'マネージャで使用したいエンコーディングを選択してください。MODxは多くのエンコードでテストを行っていますが、全てではありません。ほとんどの言語では、デフォルト設定のUTF-8をお使いください。';
...
$_lang["configcheck_cache_msg"] = 'MODxは、キャッシュディレクトリにキャッシュファイルを書き込みことができませんでした。MODxは動作しますが、キャッシュファイルは保存されません。この問題を解消するには、 /_cache/ ディレクトリを書き込み可能にをしてください。';
$_lang["configcheck_configinc"] = '設定ファイルがまだ書込可能になっています';
$_lang["configcheck_configinc_msg"] = '悪意ある人たちはあなたのサイトや関連するあらゆるものをぶち壊すことができてしまいます。<strong>本当です！</strong> あなたの設定ファイル (/manager/includes/config.inc.php) を書込み不可にしてください！';
$_lang["configcheck_default_msg"] = 'よくわからない警告が見つかりました。これは変です。';
$_lang["configcheck_errorpage_unpublished"] = 'MODx設定で設定されたエラーページは公開されていません。';
$_lang["configcheck_errorpage_unpublished_msg"] = 'これはあなたが設定したエラーページが誰でも見れるようにはなっていない事を意味します。エラーページは全ての人「Public」に公開するようにしてください。';
...
$_lang["configcheck_unauthorizedpage_unpublished_msg"] = 'これはあなたが設定した権限外告知のページが誰でも見れるようにはなっていない事を意味します。権限外告知ページは全ての人「Public」に公開するようにしてください。';
...
$_lang["confirm_delete_file"] = '本当にこのファイルを削除してもよろしいですか?\n\n削除することであなたのサイトが正常に動作しない場合もあります! 他に影響を及ぼさないことをあなたが理解している場合のみ削除してください。';
...
$_lang["confirm_remove_locks"] = 'ユーザはしばしばドキュメント、テンプレート、スニペット、プラグインなどを編集中にブラウザを閉じることがあります。するとこれらのアイテムは編集ロックされた状態で放置されることになります。OKをクリックすることで現在ロック状態にあるリソースを全て解放することができます。\n\n続けますか?';
...
$_lang["database_table_clickbackup"] = '選択した表をバックアップ、ダウンロード';
...
$_lang["deselect_metatags"] = 'METAタグのクリアs';
$_lang["disabled"] = '停止';
...
$_lang["document_opt_contentdispo_help"] = 'この内容の配置フィールドを用いてウェブブラウザがこのドキュメントをどう扱うか指定する事ができます。「Attachment」を選択するとウェブブラウザはドキュメントをダウンロードファイルとみなします。';
...
$_lang["failed_login_message"] = 'ここではユーザアカウントがロックされるまで何回ログイン失敗を許すか回数を入力します。';
...
$_lang["forgot_password_email_instructions"] = 'そのページの「私のアカウント」メニューからパスワードを変更する事ができます。';
$_lang["forgot_password_email_intro"] = 'あなたのアカウントへのパスワード変更リクエストを受け付けました。';
$_lang["forgot_password_email_link"] = '処理を完了するためここをクリックしてください。';
$_lang["forgot_your_password"] = 'パスワード紛失';
$_lang["friday"] = '金曜';
$_lang["friendlyurlsprefix_message"] = 'ユーザフレンドリーURLの接頭辞を指定してください。例えば接頭辞を「page」に設定した場合、URLの /index.php?id=2 は、 /page2.htmlというユーザフレンドリーURLに変換されます(接尾辞が .htmlに設定されていると仮定して)。このように、ユーザ(そしてサーチエンジン)が、あなたのサイトを閲覧するためのリンクを指定することができます。';
...
$_lang["import_params_msg"] = '上のドロップダウンメニューからモジュール名を選択する事により、他のモジュールのパラメータや設定をインポートする事ができます。 <b>注意:</b>メニューにモジュールが表示されるためには、このプラグインやスニペットがモジュール依存リストにありモジュールの共有パラメータ使用がONである必要があります。 ';
...
$_lang["link_attributes_help"] = 'このページへのリンクに属性を付加する事ができます。\n\n例：target=&ldquo;_blank&rdquo;、rel=&ldquo;next&rdquo;';
...
$_lang["manager"] = 'マネージャ';
$_lang['manager_lockout_message'] = 'あなたはマネージャにログインしています。セッションを閉じたいときは「ログアウト」ボタンをクリックしてください。<p />マネージャホームあるいはスタート画面に行きたいときは「ホーム」ボタンをクリックします。';
...
$_lang["messages_no_messages"] = '受信箱にメッセージはありません。';
...
$_lang["metatag_notice"] = 'METAタグに関しての詳細は<a href="http://www.html-reference.com/META.htm" target="_blank">HTML Reference Guide</a>を参照してください。このリストは指定可能なMETAタグを網羅してはいません。';
...
$_lang["mgrlog_query_msg"] = 'マネージャのログに対する検索条件を指定します。日付でログエントリを選別する事ができますが、日付の条件指定は、「以上」「未満」です。例: 2004年1月1日のログを含めるには、 「この日付から」を 01-01-2004 に、「この日付の前」を 02-01-2004 にしてください。<br /><br />
...
$_lang["mgrlog_sortinst"] = '各欄のヘッダをクリックする事で表の表示順を変更できます。ログが溜まってきたときには、<a href="index.php?a=55">empty</a> をクリックする事でログを全部消去できます。 この操作は即実行され元に戻せません。';
...
$_lang["new_file_permissions_message"] = 'ファイルマネージャで新しいファイルをアップロードしたとき、ファイルマネージャは新ファイルをここで設定したアクセス許可権限に変更します。この機能はIIS上など一部の環境では動作しません。その場合はsshなどを通じて別途アクセス許可を変更してください。';
$_lang["new_file_permissions_title"] = '新ファイルのアクセス許可';
$_lang["new_folder_permissions_message"] = 'ファイルマネージャで新しいフォルダをを作成したとき、ファイルマネージャは新フォルダをここで設定したアクセス許可権限に変更します。この機能はIIS上など一部の環境では動作しません。その場合はsshなどを通じて別途アクセス許可を変更してください。';
...
$_lang["no_docs_pending_unpublishing"] = '非公開予定のドキュメントはありません。';
...
$_lang["page_data_contentType_help"] = 'このドキュメントのコンテンツタイプを選択してください。コンテンツタイプが分からない場合は、text/htmlのままにしてください。';
...
$_lang["publish_events"] = 'ドキュメント公開イベント';
...
$_lang["resolve_hostnames_message"] = '訪問者があなたのサイトを訪れた時、MODxでIPアドレスからホスト名への解決を行いますか? ホスト名の解決を行うことによりサーバの負荷が多少増加します。しかし、ビジターがこのことに気付くことはありません。';
...
$_lang["snippet_msg"] = 'ここではスニペットを追加/編集することができます。スニペットは「生の」PHPコードです。スニペットの出力をテンプレートの特定の場所に表示したい場合は、テンプレート内にスニペット名を記述する必要があります - 基本的に、スニペットのアウトプットはスニペット自身と同じ名前で保存された変数を必要とします。';
...
$_lang["table_hoverinfo"] = 'テーブル名にマウスをかざすとテーブルの機能について簡単な説明を見る事ができます（全てのテーブルに<i>説明</i>があるわけではありません）。';
...
$_lang["unpublish_events"] = 'ドキュメント公開取消のイベント';
...
$_lang["user_changeddata"] = 'あなたのユーザ情報が変更されました。もう一度ログインしなおしてください。';
...
$_lang["user_upload_message"] = ' あなたがこのユーザに対してこのカテゴリに含まれるファイルタイプのアップロードを禁じたい場合、「MODx設定を使用」をチェックせず、このフィールドをブランクにしておいてください。';
