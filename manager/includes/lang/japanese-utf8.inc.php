...
$_lang["configcheck_cache_msg"] = 'キャッシュファイルを保存できません。'.CMS_NAME.'は動作しますが、キャッシュが利用できないため軽快なレスポンスを得られません。キャッシュを有効にするためには /cache/ ディレクトリを書き込み可能にしてください。';
...
$_lang["configcheck_installer_msg"] = '/install/ ディレクトリに'.CMS_NAME.'のインストーラーが残されています。攻撃を受ける可能性がありますので、すぐに削除してください。';
...
$_lang["configcheck_register_globals_msg"] = 'クロスサイトスクリプティング攻撃(XSS)を受けやすい脆弱性があります。XSS攻撃は'.CMS_NAME.'本体に限らず、サードパーティによって開発されるスニペットやプラグインも攻撃対象となります。register_globals on の環境を特に必要としない場合は、.htaccessまたはphp.iniによって OFF にすることを強くおすすめします。';
...
$_lang["eventlog_msg"] = 'イベントログは'.CMS_NAME.'のシステムが出力する告知・警告・エラーの各メッセージを記録するものです。';
...
$_lang["file_folder_chmod_error"] = 'アクセス権を変更できません。'.CMS_NAME.'以外の方法でアクセス権を変更してください。';
...
$_lang["friday"] = '金';
$_lang["friendly_alias_message"] = 'フレンドリーURLを使用して、かつリソースがエイリアスを持つ場合、エイリアスは常にフレンドリーURLに優先します。このオプションを「はい」に設定することで、フレンドリーURLの接頭辞と接尾辞もエイリアスに適用されます。例えば、リソースのIDが1、エイリアスが「introduction」、接頭辞が「」(空白)、接尾辞が「.html」の場合、このオプションを「はい」にすると、URLは「introduction.html」となります。 エイリアスを設定していない場合は、'.CMS_NAME.'が「1.html」というリンクを生成します。';
$_lang["friendly_alias_title"] = 'フレンドリエイリアス ';
$_lang["friendlyurls_message"] = '静的URI機能（フレンドリーURL）を有効にします。この機能は'.CMS_NAME.'がApache上で動作している時のみ有効です。この機能を動作させるためには'.CMS_NAME.'インストールディレクトリのht.accessファイルを.htaccessにリネームしてください。動作しない場合は.htaccessを開いて、コメントアウトされている「Options +FollowSymlinks」の記述を有効にしてみてください。';
...
$_lang["guid"] = 'GUID';
$_lang["help"] = 'ヘルプ';
$_lang["help_msg"] = '<p><a href="http://modxcms-jp.com/bb/" target="_blank">'.CMS_NAME.'日本公式フォーラム</a>でサポートを得ることができます。 また、逐次執筆が進められている<a href="http://'.CMS_DOMAIN.'/documentation" target="_blank">'.CMS_NAME.'ドキュメントとガイド（英語）</a>があり、'.CMS_NAME.' のほぼすべてに関して記されています。</p>';
...
$_lang["import_site_maxtime_message"] = CMS_NAME.'がサイトのインポート処理に使用できる最大秒数を指定します。（<a href="http://www.google.com/search?q=php+set_time_limit&hl=ja&lr=lang_ja" target="_blank">PHPの指定処理秒数</a>を上書きします）「0」を指定すると処理時間を制限しませんが、想定外の負荷が発生する恐れがあるため注意してください。';
...
$_lang["loading_page"] = CMS_NAME.'はページをロードしています。お待ちください';
...
$_lang["login_message"] = CMS_NAME.'へようこそ。ログイン名とパスワードは大文字・小文字を区別します。';
...
$_lang["manager_theme_message"] = '管理画面で使用するデザインテーマを選択してください。この選択肢を増やすには<a href="http://'.CMS_DOMAIN.'/extras/repository/packages/?repository=7" target="_blank">'.CMS_NAME.'開発元の配布ページ</a>から入手して /manager/media/style/ ディレクトリに置く必要があります。';
...
$_lang["modules"] = 'モジュール';
$_lang["modx_news"] =CMS_NAME.'の最新情報';
$_lang["modx_news_tab"] =CMS_NAME.'の最新情報';
$_lang["modx_news_title"] =CMS_NAME.'の最新情報';
$_lang["modx_security_notices"] =CMS_NAME.' セキュリティ情報';
$_lang["modx_version"] = CMS_NAME.'バージョン';
$_lang["monday"] = '月';
$_lang["move"] = '移動';
...
$_lang["php_version_check"] = CMS_NAME.'はPHPバージョン4.0.3またはそれ以上で動作します。インストールされているPHPをアップグレードしてください。';
$_lang["plugin"] = 'プラグイン';
...
$_lang["rb_base_url_message"] = 'ファイルブラウザディレクトリ(通常はassetsディレクトリ)のパスです。「ファイルブラウザディレクトリの物理パス」が参照する場所と同じですが、ここでは'.CMS_NAME.'設置ディレクトリを基準とした相対パスを記述します。';
...
$_lang["resource_type_message"] = '「ウェブリンク」は、ネット上のリソース(ウェブページや画像・各種ファイルなど)を'.CMS_NAME.'のリソースとして扱うものです。これにより、他システムで作った掲示板などを'.CMS_NAME.'管理下にまとめることができます。コンテントタイプは「text/html」、「リソースの出力」は「ブラウザ内に通常表示」としてください。';
...
$_lang["role"] = 'ロール(役割)';
$_lang["role_about"] = '「'.CMS_NAME.'について」ページの表示';
...
$_lang["role_management_msg"] = 'ロール(役割)ごとに、利用可能な機能を選択・設定します。ロールは役割・役職のようなもので、たとえば、部長・一般社員・アルバイトの3者の異なる権限をシステム的に表現しています。'.CMS_NAME.'では、管理画面にアクセスする全てのユーザが何らかのロール(役割)を持ちます。';
...
$_lang["rss_url_news_message"] = '管理画面トップの「'.CMS_NAME.' News」のフィードURL';
$_lang["rss_url_news_title"] = 'RSS ニュースフィード';
$_lang["rss_url_security_default"] = 'http://feeds2.feedburner.com/modxjpsec';
$_lang["rss_url_security_message"] = '管理画面トップの「'.CMS_NAME.' Security」のフィードURL';
...
$_lang["show_meta_message"] = 'リソース編集画面にMETA Keywordsタブを表示します。過去の'.CMS_NAME.'との互換性が必要な場合を除き、当設定は使い勝手の問題により推奨しません。';
...
$_lang["tmplvars_management_msg"] = '<h3 style="font-weight:bold;">テンプレート変数の管理</h3><p>一般的なCMSでいうところのカスタムフィールドに相当するもので、'.CMS_NAME.'はこの機能が非常に柔軟・強力です。「タイトル」や「内容」などの既定の項目に加え、投稿画面上に自由に項目を追加できます。テンプレート内の任意の場所に <strong>[*テンプレート変数名*]</strong> と記述すると値を出力できます。詳しくは<a href="index.php?a=9">ヘルプ</a>を参照してください。</p>';
...
$_lang["to"] = '-';
$_lang["tools"] = 'ツール';
$_lang["top_howmany_message"] = '訪問レポートの「トップ ...」に何件表示するか指定します。(※訪問レポートの機能は現在の'.CMS_NAME.'には実装されていません)';
...
$_lang["webpwdreminder_message"] = 'ウェブユーザーからパスワード変更の要求があった時に、'.CMS_NAME.'は新パスワードとアクティベーション方法を記載したメールを送信することができます。このメールの内容は変更できます。<br /><strong>参考:</strong> 下記の差し込み文字列は送信時に自動的に置き換えられます。<br /><br />[+sname+] - サイトの名前<br />[+saddr+] - サイト管理者のメールアドレス<br />[+surl+] - サイトのURL<br />[+uid+] - ログイン名<br />[+pwd+] - パスワード<br />[+ufn+] - フルネーム';
$_lang["webpwdreminder_title"] = 'パスワード変更<br />リクエストメール ';
$_lang["websignupemail_message"] = 'ユーザー自らがウェブユーザーアカウントを作成した時に、'.CMS_NAME.'はログイン名とパスワードを記載したメールを送信することができます。このメールの内容は変更できます。<br /><strong>参考:</strong> 下記の差し込み文字列は送信時に自動的に置き換えられます。<br /><br />[+sname+] - サイトの名前<br />[+saddr+] - サイト管理者のメールアドレス<br />[+surl+] - サイトのURL<br />[+uid+] - ログイン名<br />[+pwd+] - パスワード<br />[+ufn+] - フルネーム';
...
$_lang["welcome_title"] = CMS_NAME.'へようこそ';
$_lang["which_editor_message"] = 'デフォルトで使用するリッチテキストエディターを選択してください(※<a href="http://'.CMS_DOMAIN.'/extras/repository/packages/Content" target="_blank">'.CMS_NAME.'のダウンロードページ</a>には追加でインストールできるリッチテキストエディターがあります)。';
