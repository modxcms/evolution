...
$_lang["access_permissions_docs_message"] = 'このドキュメントを所属させたいドキュメントグループを選択してください。アクセスを制限する必要がなければ「全ドキュメントグループ」にチェックを入れておきます。';
...
$_lang["access_permissions_introtext"] = 'ユーザーグループとドキュメントグループを管理します。ここでユーザーグループまたはドキュメントグループを作ると、ユーザ編集画面またはドキュメント編集画面で所属グループを選択できるようになります。また、ユーザーグループとドキュメントグループをここで関連付けることができます。つまり誰がどのドキュメントをという関連付けをここで行ないます。利用できる機能を制限・管理する「ロール」と違い、当機能ではコンテンツ(ドキュメント)対象の権限を管理します。<br /><a href="http://whatsnew.cc/modx/manager/index.php?a=86">ロール管理</a>とは区別されているため、同じユーザーグループ内に「強い権限を持つユーザー」や「限定された権限を持つユーザー」を自由に混在させることができます。<br />※「ツール」→「グローバル設定」→「詳細設定」の「アクセス権限設定の使用」を「いいえ」にすると、グループ設定を経由しない運用になります。信用できる少数のメンバーでシンプルにサイトを管理運用したい場合は参考にしてください。';
...
$_lang["configcheck_configinc_msg"] = '今の状態では、悪意ある人がこのサイトを壊すことができてしまいます。設定ファイル (/assets/config.inc.php)のパーミッションを404などに設定し、書込み不可にしてください。';
...
$_lang["confirm_duplicate_document"] = 'このドキュメントを複製してもよろしいですか？配下のフォルダやドキュメントも複製されます。';
$_lang["confirm_duplicate_record"] = 'このレコードを複製してもよろしいですか?';
$_lang["confirm_empty_trash"] = '全ての削除済みドキュメントを完全に消去しますか?\n\n続けますか?';
$_lang["confirm_load_depends"] = '変更を保存せずに「依存性の管理」画面をロードしてもよろしいですか?';
...
$_lang["file_folder_not_deleted"] = 'フォルダを削除できません。フォルダを削除する前に中身が空であることを確認してください!';
...
$_lang["mail_check_timeperiod_message"] = 'メールチェックの間隔を「秒」で指定します';
$_lang["manager"] = '管理画面';
...
$_lang["module_msg"] = 'ここでモジュールの追加・編集ができます。';
...
$_lang["no_docs_pending_publishing"] = '公開を予定しているドキュメントはありません。';
...
$_lang["rb_base_dir_message"] = 'リソースディレクトリへの物理パスを入力してください。通常、この設定は自動的に生成されます。IISを使用していると、MODxが自分自身のパスを判別できないことがあるためリソースブラウザがエラーを表示することがあります。この場合、イメージディレクトリなどのパスをここに入力することができます（パスはWindowsのエクスプローラで見るものです）。 <strong>注意:</strong> リソースブラウザが全てを扱えるようにするためには、リソースディレクトリにイメージや文書、フラッシュ他のメディアを格納するサブフォルダを含むべきです。';
...
$_lang["rb_message"] = '「はい」を選択するとリソースブラウザを使用できます。これによりユーザーはサーバー上の画像やフラッシュなどのメディアファイルをブラウズしたり、ファイルをサーバーにアップロードすることがきるようになります。';
$_lang["rb_title"] = 'リソースブラウザを使用:';
$_lang["rb_webuser_message"] = 'ウェブユーザーにリソースブラウザの利用を許可するかどうかを選択します。 <strong>警告:</strong> ウェブユーザーにリソースブラウザの利用を許可するということは、管理画面ユーザーが利用するファイルを彼らに見せることになります。従って、信用できるウェブユーザーにのみ許可するべきでしょう。';
...
$_lang["settings_after_install"] = '<p><strong style="color:red;">新規インストール直後のため、各種設定を確認し保存する必要があります。設定を確認した後、データベースを更新するために「保存」ボタンをクリックしてください。</strong></p><p>※この設定は後で自由に変更できます(ツール→グローバル設定)。今の時点ではサイトステータス(公開・準備中など)の設定のみ確認しておくといいでしょう。</p>';
...
$_lang["user_zip"] = '郵便番号';
$_lang["use_alias_path_message"] = 'このオプションを「はい」に設定すると、ドキュメントのエイリアスがパスとして表示されます。例えば、「child」というエイリアスを持ったドキュメント「parent」というエイリアスのフォルダの下に配置されていると、エイリアスパスは「/parent/child.html」になります。<br /><strong>注意: このオプションを「はい」にするとき、つまりエイリアスパスを使用するときは、ドキュメントから参照するものは全て（画像とかCSSとかJavaScriptなど）絶対パスで指定する必要があります。例えば、assets/imagesに対しては/assets/imagesと指定しなければなりません。baseタグを利用すると、この問題をスマートに解決できます。</strong>';
