...
$_lang["access_permissions_link_to_group"] = 'ドキュメントグループ';
...
$_lang["charset_message"] = '管理画面で使用したいエンコードを選択してください。日本語も含め、ほとんどの言語ではデフォルト設定のUTF-8が利用できます。<br />※EUCを選択する場合はEUC版のlangファイルをフォーラムより入手しインストールする必要があります。またShiftJISは十分に検証されていません。';
...
$_lang["configcheck_register_globals_msg"] = 'この状態は、クロスサイトスクリプティング攻撃(XSS)を受けやすい脆弱性があります。XSS攻撃はMODx本体に限らず、サードパーティによって開発されるスニペットやプラグインも攻撃対象となります。register_globals on の環境を特に必要としない場合は、.htaccessまたはphp.iniによって OFF にしてください';
...
$_lang["filemanager_path_message"] = '多くの場合IISではドキュメントルートの設定が適切に得られません。ドキュメントルート情報はファイルマネージャーで扱える範囲を決定します。ファイルマネージャーの使用で問題がある場合は、この項目がインストールしたMODxのルートディレクトリを指しているか確認してください。';
...
$_lang["friendly_alias_message"] = 'フレンドリーURLを使用して、かつドキュメントがエイリアスを持つ場合、エイリアスは常にフレンドリーURLに優先します。このオプションを「はい」に設定することで、フレンドリーURLの接頭辞と接尾辞もエイリアスに適用されます。例えば、ドキュメントのIDが1、エイリアスが「introduction」、接頭辞が「」、接尾辞が「.html」の場合、このオプションを「はい」にすると、URLは「introduction.html」となります。 エイリアスを設定していない場合は、MODxが「1.html」というリンクを生成します。';
...
$_lang["language_message"] = '管理画面の使用言語を選択してください';
...
$_lang["manager"] = '管理画面';
$_lang["manager_direction_message"] = '管理画面でのテキストの流れる方向を指定します。「ltr」は、左から右。「rtl」は、右から左です。主にアラビア語圏などで利用する設定です。';
...
$_lang["manager_permissions"] = '管理画面へのアクセス許可';
...
$_lang["mgr_access_permissions"] = '管理画面アクセス許可';
$_lang["mgr_login_start"] = '管理画面ログイン開始ページ';
$_lang["mgr_login_start_message"] = 'ユーザーが管理画面にログインしたとき、リダイレクトしたいドキュメントのIDを入力します。<strong>注意: 入力するIDは、既に存在するドキュメントIDであり、ドキュメントは公開されていてこのユーザーがアクセス可能である必要があります!</strong>';
...
$_lang["rb_webuser_message"] = 'ウェブユーザーにリソースブラウザの利用を許可するかどうかを選択します。 <strong>警告:</strong> ウェブユーザーにリソースブラウザの利用を許可するということは、管理画面ユーザーが利用するファイルを彼らに見せることになります。従って、本当に信用できるウェブユーザーにのみ、許可するべきでしょう。';
...
$_lang["udperms_message"] = 'アクセス権限設定を有効にすると、ユーザーの編集権限をユーザーグループごと・ドキュメントグループごとに割り当てることができます。まず<a href="index.php?a=40" target="_self">管理画面へのアクセス許可</a>を開き、ユーザーグループとドキュメントグループを作ってください。次に、それぞれのユーザーグループとドキュメントグループを関連付けてください。最後に、個別のユーザーをユーザーグループに、個別のドキュメントをドキュメントグループに割り当ててください。この設定を「はい」にした場合、全てのドキュメントを編集する権限を持つのは初期状態では管理者のみとなります。';
...
$_lang["user_upload_message"] = ' このユーザーに対してこのカテゴリーに含まれるファイルタイプのアップロードを禁じたい場合、「MODx設定を使用」をチェックせず、このフィールドを空白にしておいてください。';
