...
$_lang["access_permissions_docs_collision"] = 'Because the Web User Groups and Manager User Groups conflict for your Role, this document will be public. Contact a site administrator for further help.';//未訳
...
$_lang["access_permissions_introtext"] = 'ユーザーグループとドキュメントグループを管理します。ここでユーザーグループまたはドキュメントグループを作ると、ユーザ編集画面またはドキュメント編集画面で所属グループを選択できるようになります。また、ユーザーグループとドキュメントグループをここで関連付けることができます。つまり誰がどのドキュメントをという関連付けをここで行ないます。利用できる機能を制限・管理する「ロール」と違い、当機能ではコンテンツ(ドキュメント)対象の権限を管理します。<br /><a href="index.php?a=86">ロール管理</a>とは区別されているため、同じユーザーグループ内に「強い権限を持つユーザー」や「限定された権限を持つユーザー」を自由に混在させることができます。<br />※「ツール」→「グローバル設定」→「詳細設定」の「アクセス権限設定の使用」を「いいえ」にすると、グループ設定を経由しない運用になります。信用できる少数のメンバーでシンプルにサイトを管理運用したい場合は参考にしてください。';
...
$_lang["credits_everaldo"] = 'Everaldo の素晴らしいクリスタルアイコンセットを多数利用しています。';
...
$_lang["credits_webfx"] = 'MODx は Webfx の素晴らしいスクリプトを多く利用しています。';
...
$_lang["defaultsearch_message"] = '新しく作成するドキュメントをデフォルトで検索対象とする場合は「はい」を選択してください。';
$_lang["defaultsearch_title"] = '検索可能デフォルト';
$_lang["defaulttemplate_message"] = '新規に作成するドキュメントのデフォルトテンプレートを指定します。テンプレートはドキュメント単位で自由に選択できます。';
...
$_lang["document_long_title_help"] = 'ページのタイトルです。titleタグなどで利用します。MODxタグ：[*longtitle*]';
$_lang["document_metatag_help"] = 'このドキュメントに付加したいMETAタグやキーワードを選択してください。CTRLキーを押しながらクリックすると複数のタグやキーワードを選択できます。選択元とするMETAタグ・キーワードの作成・管理は「エレメント」→<a href="index.php?a=81" target="_self">「METAタグとキーワードの管理」</a>で行ないます。';
...
$_lang["document_opt_menu_index_help"] = 'メニューオーダーは、メニュースニペット内でドキュメントを並び替えるために使用できます。また、スニペット内で他の目的に使用することもできます。';
...
$_lang["document_permissions_error"] = 'Assign this Document to at least one Document Group which you can access.';//未訳
...
$_lang["document_title_help"] = 'ドキュメントの名称/タイトルを入力してください。バックスラッシュは使用できません。管理画面のツリーで表示されるため短めにするといいでしょう(MODxタグ：[*pagetitle*])';
...
$_lang["link_attributes_help"] = 'このページ内の任意のリンクに属性を一律付加できます。\n\n例：target=&quot;_blank&quot; ・rel=&quot;lightbox&quot;\nドキュメント変数：[*link_attributes*]';
...
$_lang["module_msg"] = 'ここでモジュールの追加・編集ができます。A Module is a collection of Elements (e.g. plugins, snippets, etc).';//※要確認
$_lang["module_name"] = 'モジュール名';
$_lang["module_resource_msg"] = 'このモジュールが依存するエレメントを追加・削除します。新しいエレメントを追加するには下のボタンのひとつをクリックします。';
$_lang["module_resource_title"] = 'モジュール依存';
$_lang["module_title"] = 'モジュールの作成/編集';
$_lang["module_viewdepend_msg"] = 'モジュールが依存するエレメントの状況を見ることができます。依存性を変更するには「依存性の管理」ボタンをクリックします';
$_lang["modules"] = 'モジュール';
...
$_lang["new_folder_permissions_message"] = '管理画面を通じて新しいフォルダをを作成したとき、ここで設定したパーミッションに変更します。この機能はIIS上など一部の環境では動作しません。';
...
$_lang["rb_base_dir_message"] = 'ファイルブラウザディレクトリ(アセットディレクトリ)への物理パスを入力してください。通常この設定は自動的に生成されますが、IISなど一部のサーバでは正常に検出されないことがあります。この場合、イメージディレクトリなどのパスをここに入力することができます（パスはWindowsのエクスプローラで見るものです）。 <strong>注意:</strong> ファイルブラウザが全てを扱えるようにするためには、ファイルディレクトリにイメージや文書、Flash他のメディアを格納するサブフォルダを含むべきです。';//※要確認
...
$_lang["rb_message"] = '「はい」を選択するとファイルブラウザを使用できます。これによりユーザーはサーバー上の画像やFlashなどのメディアファイルをブラウズしたり、ファイルをサーバーにアップロードすることがきるようになります。';
$_lang["rb_title"] = 'ファイルブラウザを使用:';
$_lang["rb_webuser_message"] = 'ウェブユーザーにファイルブラウザの利用を許可するかどうかを選択します。 <strong>警告:</strong> ウェブユーザーにファイルブラウザの利用を許可するということは、管理画面ユーザーが利用するファイルを彼らに見せることになります。従って、信用できるウェブユーザーにのみ許可するべきでしょう。';
...
$_lang["settings_strip_image_paths_message"] = 'この設定を「いいえ」にした場合、MODxはファイルブラウザで選択したファイル（画像・Flash・その他メディアファイルなど）のsrc指定を絶対パスで書き出します。それに対し相対パスは、インストール済みのMODxを移動したい時に大変便利です(例えば、開発サイトから正式サイトへの移行など)。このことに関して理解に自信がない場合は「はい」にしておくことをお勧めします。';
...
$_lang["showing"] = '表示中';
$_lang["signupemail_message"] = 'ユーザ管理画面でユーザーアカウントを作成した時に、ユーザー名とパスワードを記載したメールを送信することができます。このメールの内容を設定してください。<br /><strong>注意:</strong> 下記の差し込み文字列は送信時に自動的に置き換えられます。<br /><br />[+sname+] - サイトの名前, <br />[+saddr+] - サイト管理者のメールアドレス, <br />[+surl+] - サイトのURL, <br />[+uid+] - ユーザーのログイン名, <br />[+pwd+] - ユーザーのパスワード, <br />[+ufn+] - ユーザーのフルネーム. ';
...
$_lang["snippets_available"] = 'ページで利用可能なスニペット';
$_lang["sort_asc"] = '昇順';
$_lang["sort_desc"] = '降順';
...
$_lang["webpwdreminder_message"] = 'ウェブユーザーからパスワード変更の要求があった時に、MODxは新パスワードとアクティベーション方法を記載したメールを送信することができます。このメールの内容を設定してください。<br /><strong>注意:</strong> 下記の差し込み文字列は送信時に自動的に置き換えられます。<br /><br />[+sname+] - サイトの名前, <br />[+saddr+] - サイト管理者のメールアドレス, <br />[+surl+] - サイトのURL, <br />[+uid+] - ユーザーのログイン名, <br />[+pwd+] - ユーザーのパスワード, <br />[+ufn+] - ユーザーのフルネーム. ';
$_lang["webpwdreminder_title"] = 'パスワード変更<br />リクエストメール:';
$_lang["websignupemail_message"] = 'ユーザー自らがウェブユーザーアカウントを作成した時に、MODxはユーザー名とパスワードを記載したメールを送信することができます。このメールの内容を設定してください。<br /><strong>注意:</strong> 下記の差し込み文字列は送信時に自動的に置き換えられます。<br /><br />[+sname+] - サイトの名前, <br />[+saddr+] - サイト管理者のメールアドレス, <br />[+surl+] - サイトのURL, <br />[+uid+] - ユーザーのログイン名, <br />[+pwd+] - ユーザーのパスワード, <br />[+ufn+] - ユーザーのフルネーム. ';
