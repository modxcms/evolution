<?php
/**
 * Filename:       includes/lang/simple_chinese.inc.php
 * Function:       Simple Chinese language file.
 * Encoding:       gb2312
 * Author:         modx.cn leepoooo@163.com
 * Date:           2006/03/18
 * Version:        0.1 (from English Ver1.6)
 * MODx version:   0.9.1
*/

// NOTE: New language keys should added at the bottom of this page

$_lang["login_message"] = "请输入你的管理帐号，注意，大小写";
$_lang["username"] = "用户名";
$_lang["password"] = "密码";
$_lang["remember_username"] = "记住我";
$_lang["login_button"] = "登陆";
$_lang["home"] = "主页";

$_lang["my_preferences"] = "预设";
$_lang["site"] = "站点";
$_lang["launch_site"] = "浏览站点";
$_lang["refresh_site"] = "刷新站点";
$_lang["content"] = "内容";
$_lang["add_document"] = "新文档";
$_lang["add_weblink"] = "新连接";
$_lang["site_schedule"] = "站点时间表";
$_lang["visitor_stats"] = "访问者状态";
$_lang["messages"] = "消息";
$_lang["users"] = "用户";
$_lang["change_password"] = "改密码";
$_lang["new_user"] = "新用户";
$_lang["edit_user"] = "编辑用户";
$_lang["new_role"] = "建立新角色role";
$_lang["edit_role"] = "编辑角色role";
$_lang["access_permissions"] = "访问许可";
$_lang["administration"] = "管理员";
$_lang["edit_settings"] = "系统配置";
$_lang["view_sysinfo"] = "系统信息";
$_lang["edit_parser"] = "编辑剖析器";
$_lang["site_parser"] = "站点剖析器";
$_lang["action_parser"] = "作用中的剖析器";
$_lang["view_logging"] = "索引";
$_lang["manage_files"] = "文件管理";
$_lang["remove_locks"] = "Remove locks";
$_lang["resources"] = "资源";
$_lang["new_template"] = "新模板";
$_lang["edit_template"] = "编辑模板";
$_lang["new_snippet"] = "新snippet";
$_lang["edit_snippet"] = "编辑snippet";
$_lang["document_tree"] = "文件树";
$_lang["refresh_tree"] = "刷新文件树";
$_lang["expand_tree"] = "展开文件列表";
$_lang["collapse_tree"] = "收拢文件列表";
$_lang["hide_tree"] = "隐藏树";
$_lang["show_tree"] = "显示列表树";
$_lang["restore_tree"] = "重置列表树 (default width)";
$_lang["help"] = "帮助";
$_lang["credits"] = "Credits";
$_lang["about"] = "关于MODX";
$_lang["rebuild"] = "Rebuild";
$_lang["menu"] = "菜单";
$_lang["tree"] = "树目";
$_lang["manager"] = "管理员";
$_lang["test_page"] = "测试页面";
$_lang["logout"] = "退出";
$_lang["nojava"] = "Java seems to be disabled. Click <a href='index.php?a=1&f=9&javaDis=1'>here</a> to load the non-java menu.";
$_lang["confirm_empty_trash"] = "您将永久的删除这些文件?\\n\\n继续吗？"; // changed 06 RC2
$_lang["unable_set_parent"] = "Unable to set new parent!";
$_lang["confirm_delete_document"] = "确定要删除文件吗？\\n这些文件的子目录都会被删除！";
$_lang["already_deleted"] = "已经被删除.";
$_lang["not_deleted"] = "没被删除.";
$_lang["confirm_undelete"] = "\\n\\nAny children documents deleted at the same time as this document will also be undeleted, but children documents deleted at an earlier time will still be deleted.";
$_lang["confirm_publish"] = "\\n\\nPublishing this document now will remove any (un)publishing dates that may have been set. If you wish to set or keep publish or unpublish dates, please choose to 'edit' the document instead.\\n\\nProceed?";
$_lang["confirm_unpublish"] = "\\n\\nUn-publishing this document now will remove any (un)publishing dates that may have been set. If you wish to set or keep publish or unpublish dates, please choose to 'edit' the document instead.\\n\\nProceed?";
$_lang["view_document"] = "查看文档";
$_lang["edit_document"] = "编辑文档";
$_lang["move_document"] = "移动文档";
$_lang["create_document_here"] = "在这里建立文档";
$_lang["create_weblink_here"] = "在这里建立连接";
$_lang["delete_document"] = "删除文件";
$_lang["undelete_document"] = "恢复删除的文件";
$_lang["publish_document"] = "发布文档";
$_lang["unpublish_document"] = "取消发布文档";
$_lang["published_document"] = "This is a published document.";
$_lang["unpublished_document"] = "This is an un-published document.";
$_lang["deleted_document"] = "This is a deleted document.";
$_lang["published_folder"] = "This is a published folder.";
$_lang["unpublished_folder"] = "This is an un-published folder.";
$_lang["deleted_folder"] = "This is a deleted folder.";
$_lang["published_weblink"] = "This is a published weblink.";
$_lang["unpublished_weblink"] = "This is an un-published weblink.";
$_lang["deleted_weblink"] = "This is a deleted weblink.";
$_lang["published_weblink_folder"] = "This is a published weblink folder.";
$_lang["unpublished_weblink_folder"] = "This is an un-published weblink folder.";
$_lang["deleted_weblink_folder"] = "This is a deleted weblink folder.";
$_lang["recycle_bin"] = "回收站"; // obsolete
$_lang["empty_recycle_bin"] = "清空回收站"; // changed 06 rc 2
$_lang["deleted_in_tree"] = "Show deleted in tree"; // obsolete
$_lang["deleted_in_recycle_bin"] = "Show deleted in Recycle Bin"; // obsolete
$_lang["recycle_bin_empty"] = "The Recycle Bin is empty."; // obsolete
$_lang["loading_doc_tree"] = "Loading document tree...";
$_lang["loading_menu"] = "加载菜单......";
$_lang["loading_menu_tree"] = "加载菜单和文件树...";
$_lang["working"] = "工作中...";
$_lang["confirm_remove_locks"] = "Users sometimes close their browser while editing documents, templates, snippets or parsers, possibly leaving the item they were editing in locked state. By pressing OK you can remove ALL locks currently in place.\\n\\nProceed?";
$_lang["welcome_title"] = "欢迎来到MODx内容管理";
$_lang["welcome_messages"] = "你有 <b>%d</b> 消息,  <b>%s</b>没读";
$_lang["activity_title"] = "Recently edited/ created documents";
$_lang["activity_message"] = "This list shows the last documents you created or edited:";
$_lang["no_activity_message"] = "You have not yet created or edited any documents.";
$_lang["yourinfo_title"] = "你的信息";
$_lang["yourinfo_message"] = "显示关于你的信息:";
$_lang["yourinfo_username"] = "你的登陆名:";
$_lang["yourinfo_role"] = "你的角色是:";
$_lang["yourinfo_previous_login"] = "最后登陆日期:";
$_lang["yourinfo_total_logins"] = "总登陆次数:";
$_lang["onlineusers_title"] = "在线用户";
$_lang["onlineusers_message"] = "显示最近20分种内的在线用户 (现在时间是：";
$_lang["onlineusers_user"] = "用户";
$_lang["onlineusers_userid"] = "用户ID";
$_lang["onlineusers_ipaddress"] = "用户IP地址";
$_lang["onlineusers_lasthit"] = "最后点击";
$_lang["onlineusers_action"] = "Action";
$_lang["configcheck_title"] = "配置检查";
$_lang["configcheck_notok"] = "一个或多个配置未通过设置: ";
$_lang["configcheck_ok"] = "检查通过，没有错误报告";
$_lang["configcheck_warning"] = "配置警告:";
$_lang["configcheck_admin"] = "请联系系统管理员，并把消息发送给他!";
$_lang["configcheck_what"] = "这是什么意思?";
$_lang["configcheck_installer"] = "安装程序没删除";
$_lang["configcheck_installer_msg"] = "The install/ directory contains the installer for MODx. Just imagine what might happen if an evil person finds this folder and runs the installer! He'd probably not get too far, because he'll need to enter some user information for the database, but it's still best to remove this folder from your server.";
$_lang["configcheck_cache"] = "cache目录不可写";
$_lang["configcheck_cache_msg"] = "MODX不能往CACHE里写入东西. 当然MODX可以照样脱离caching运行, 将 /_cache/ 目录设置为可写就可以解决问题.";
$_lang["configcheck_lang_difference"] = "语言文件行数不正确";
$_lang["configcheck_lang_difference_msg"] = "当前选择的文件和默认的文件的语言行数不一样，这虽然不构成什么问题，但以为着你的语言文件需要更新了。";
$_lang["configcheck_default_msg"] = "发生不知名的错误.";
$_lang["clicktopin"] = "点击锁住它.";
$_lang["clicktounpin"] = "点击取消锁住状态.";
$_lang["clickonsection"] = "展开或收拢本内容.";
$_lang["edit"] = "编辑";
$_lang["save"] = "保存";
$_lang["delete"] = "删除";
$_lang["cancel"] = "退出";
$_lang["find_in_tree"] = "Find in tree"; // obsolete per 06 RC2
$_lang["preview"] = "预览";
$_lang["move"] = "移动";
$_lang["yes"] = "是";
$_lang["no"] = "否";
$_lang["online"] = "在线";
$_lang["offline"] = "离线";
$_lang["advanced"] = "高级";
$_lang["regular"] = "一般";
$_lang["showing"] = "正在显示中";
$_lang["to"] = "to";
$_lang["total"] = "总数";
$_lang["settings_title"] = "系统配置";
$_lang["serveroffset_title"] = "服务器偏移时间:";
$_lang["serveroffset_message"] = "选择你服务器所在地和你本人所在地的时间差别. 现在的服务器时间是 <b>[%s]</b>, 现在的服务器骗移时间是： <b>[%s]</b>.";
$_lang["sitename_title"] = "站点名字:";
$_lang["sitename_message"] = "输入站点名字.";
$_lang["sitestart_title"] = "网站开始:";
$_lang["sitestart_message"] = "输入作为你网站首页的文件ID号. <b>注意：请确认你输入ID号的文章是存在的并且已经发布！</b>";
$_lang["errorpage_title"] = "错误页面:";
$_lang["errorpage_message"] = "请输入请求错误的转向文章ID号. <b>注意：请确认你输入ID号的文章是存在的并且已经发布！</b>";
$_lang["sitestatus_title"] = "站点状态:";
$_lang["sitestatus_message"] = "选择'online'发布你的站点 如果你选择了 'Offline',访问者将看到'Site unavailable message站点不可访问'的消息而不能访问网站.";
$_lang["siteunavailable_title"] = "网站不可访问时的消息:";
$_lang["siteunavailable_message"] = "显示站点关闭或有错误发生时的消息 <b>Note: This message will only be displayed if the Site unavailable page option is not set.</b>";
$_lang["friendlyurls_title"] = "起用友好URLs:";
$_lang["friendlyurls_message"] = "这个设置将允许使用友好URL以便于搜索引擎的工作，这个设置只工作在Apache服务器下, 另外你将需要写一个.htaccess. 详细情况请查阅.htaccess文件。";
$_lang["friendlyurlsprefix_title"] = "friendly URLs的前缀:";
$_lang["friendlyurlsprefix_message"] = "这里你可以设置友好URL的前缀. 比如，设置了前缀是page的URL，对于 /index.php?id=2这个连接来说，他的友好URL就是 /page2.html 后缀是 .html).这就定置了你的用户和机器人能见到的地址连接."; // updated 0.6 - removed htaccess warning
$_lang["friendlyurlsuffix_title"] = "友好URLS的后缀:";
$_lang["friendlyurlsuffix_message"] = "友好URLs的后缀设置. 指定 '.html' 的话，MODX将把.html放在你的友好URLs后面."; // updated 0.6 - removed htaccess warning
$_lang["udperms_title"] = "使用访问许可:";
$_lang["udperms_message"] = "访问许可可以指定你的哪些文件可以被哪些人编辑.你需要分配你的用户到某用户组,分配你的文件到某文件组, 然后你就可以指定哪些拥护组可以访问哪些文件组了. 当你第一次指定这些设置之前只有网站管理员一人可以编辑任何文件."; // updated 0.6
$_lang["menutype_title"] = "菜单类型:";
$_lang["menutype_message"] = "决定你选择高级JAVA菜单还是普通的平板文字菜单. [Currently you can't change this as I still need to re-write the non-java menu, sorry!]";
$_lang["nologentries_title"] = "日志条目数:";
$_lang["nologentries_message"] = "输入每页显示的索引数量.";
$_lang["nomessages_title"] = "消息数量:";
$_lang["nomessages_message"] = "输入当你浏览消息时，你的消息框将显示的消息数量";
$_lang["language_title"] = "语言:";
$_lang["language_message"] = "选择后台管理语言.";
$_lang["graphtype_title"] = "图表类型:";
$_lang["graphtype_message"] = "'高级Advanced'选择在你的报表里启用抗锯齿功能, 并不代表有其他什么功能，只是你的图表报告会更好看点！";
$_lang["showgraphs_title"] = "显示图象:";
$_lang["showgraphs_message"] = "当生成访问者访问报告时候, 你希望显示列表数据在图象上吗 ？";
$_lang["showtestpage_title"] = "允许测试页:";
$_lang["showtestpage_message"] = "你可以使用测试页来测试你正在修改编写的'切片'，如果你不使用它，你就需要点击 '否No'来把它从菜单中移除.";
$_lang["enabledebug_title"] = "起用调试功能:";
$_lang["enabledebug_message"] = "选择 '是Yes' 将显示modx的其他功能报告.一般情况下你是不需要这个功能的，除非你希望解决MODX出现的一些BUG。";
$_lang["doc_data_title"] = "查看文档数据";
$_lang["page_data_title"] = "页面数据";
$_lang["page_data_general"] = "一般";
$_lang["document_title"] = "标题";
$_lang["document_description"] = "描述";
$_lang["page_data_changes"] = "改动";
$_lang["notset"] = "没有设置";
$_lang["page_data_created"] = "建立的";
$_lang["page_data_edited"] = "编辑的";
$_lang["page_data_status"] = "状态";
$_lang["page_data_published"] = "发布的";
$_lang["page_data_unpublished"] = "未发布的";
$_lang["page_data_publishdate"] = "发布时间";
$_lang["page_data_unpublishdate"] = "取消发布时间";
$_lang["page_data_cacheable"] = "使用Cache";
$_lang["page_data_searchable"] = "被搜索";
$_lang["page_data_markup"] = "Markup/ structure";
$_lang["page_data_template"] = "选用模板";
$_lang["page_data_editor"] = "选用富文本编辑器";
$_lang["page_data_folder"] = "文档是文件夹类型";
$_lang["page_data_source"] = "资源";
$_lang["page_data_notcached"] = "这个文件还没被CACHE.";
$_lang["page_data_cached"] = "<b>从cache中找来的资源:</b>";
$_lang['cleaningup'] = "清除";
$_lang['actioncomplete'] = "<b>命令完成!</b><br /> - 请等候MODX清除它.";
$_lang['confirm_preview'] = "这个是附注功能!\\n\\n它只显示保存的版本,而不是你刚编辑的版本.你仍需要预览吗？";
$_lang['edit_document_title'] = "建立/编辑文档";
$_lang['document_identification'] = "文档ID";
$_lang['weblink_message'] = "一个网页连接就是连接到外部ITERNET网的东西，在MODX中它可以作为一个文档类型：一个在其他网站上的页面或文件或图象.<p>";
$_lang['weblink'] = "网页连接";
$_lang['document_title_help'] = "输入文档标题名字. 避免使用反斜线！";
$_lang['document_description_help'] = "你可输入文档描述，这个并非必须输入。";
$_lang['document_weblink_help'] = "输入连接地址";
$_lang['document_parent'] = "文档之父本。";
$_lang['document_parent_help'] = "点上面文件夹的图标来激活或禁止父本的选择，当激活的时候，你可以从文档树中选择一文档作为本文档的父本。";
$_lang['document_content'] = "文档内容";
$_lang['document_opt'] = "发布，高速缓存和外观";
$_lang['document_opt_folder'] = "作为文件夹吗？";
$_lang['document_opt_richtext'] = "使用富文本编辑器吗？";
$_lang['document_opt_published'] = "发布吗？";
$_lang['document_opt_menu_index'] = "菜单索引号";
$_lang['document_opt_emptycache'] = "清除高速缓存吗？";
$_lang['document_opt_folder_help'] = "选择这个选项就是让本文档作为一个文件夹来发布，你不需要为此过多的担心，因为MODx 会为你自动设置好这些。";
$_lang['document_opt_richtext_help'] = "选择这个选项将让本文档能使用富文本编辑器，如果你的内容里有 JavaScript和表单，请取消这个选择而利用HTML格式来编辑，以免MODX为你自动剖析文本。";
$_lang['document_opt_published_help'] = "选择这个选项，将使你在编辑完文档后就能立即发布你的文档。";
$_lang['page_data_publishdate_help'] = "如果你要选择发布日间，你可以点这个日历图标，在弹出的日历上选择要发布的时间，当时间到的时候，程序会自动给你发布文档，当你点后一个图标时，将移除自动发布时间，程序将不自动给你发部文档了。";
$_lang['page_data_unpublishdate_help'] = "如果想设置取消发布时间，你可以点日历图标，然后在日历上选择时间，程序就会自动在你 指定的时间取消本文档的发布;当你点后边一个图标，那么取消发布的时间设置就被移除了，你的文档将永不会自动取消发布。";
$_lang['document_opt_menu_index_help'] = "菜单索引就是你菜单的序号，你可以在你的切片snippet(s)中利用他来实现排序选择等功能。";
$_lang['page_data_searchable_help'] = "选择这个选项将使文档能够被搜索到。";
$_lang['page_data_cacheable_help'] = "让这个选项被选择，你将把他同时保存在CACHE中，如果你的文档包含了切片的话，请取消这个选择！";
$_lang['document_opt_emptycache_help'] = "选择这个选择，将让MODX在保存你的文档的时候清空CACHE，这样你的访问者就看不到这个文档的老版本了。";
$_lang['page_data_template_help'] = "你可以在这里选择你的文档使用的模板。";
$_lang['warning_not_saved'] = "你的修改还没保存，你可以按 ('Cancel')继续编辑，或按('OK')离开本页面，不过你的修改将丢失而不被保存。";
$_lang['functionnotimpl'] = "对不起！";
$_lang['functionnotimpl_message'] = "这个功能没被执行完.";
$_lang['move_document_title'] = "移动文档。";
$_lang['move_document_message'] = "通过在文档目录树中选择一作为父本的文档，你可以移动一个文档包括它的子文件作为它文档的子文件，如果你选择来作为父本的文档不是文件夹类型，程序将自动把它设置成文件夹类型，请在文档目录树中选择一个新的文档作为其父本。";
$_lang['document_to_be_moved'] = "要移动的文档。";
$_lang['move_document_new_parent'] = "请在文档目录树中选择一新父本。";
$_lang['new_parent'] = "新父本";
$_lang['edit_parser_title'] = "编辑剖析器";
$_lang['edit_parser_warning'] = "<b style='color:#821517'>编辑这个将严格的要求你的站点!</b><br />你在编辑一物理的PHP代码，请一定在保存前先备份文件！";
$_lang['change_password_message'] = "输入两次你的密码，你的密码长度限制是6-15位。";
$_lang['change_password_new'] = "新密码";
$_lang['change_password_confirm'] = "重复密码";
$_lang['confirm_delete_role'] = "你确定删除这个角色?";
$_lang['role_title'] = "建立/编辑角色";
$_lang['role_name'] = "角色名字";
$_lang['role_content_management'] = "管理内容";
$_lang['role_template_management'] = "管理模板";
$_lang['role_snippet_management'] = "管理Snippet切片";
$_lang['role_user_management'] = "管理用户";
$_lang['role_udperms'] = "管理许可";
$_lang['role_role_management'] = "管理角色";
$_lang['role_config_management'] = "管理配置";
$_lang['role_frames'] = "管理请求框架";
$_lang['role_home'] = "管理主页面";
$_lang['role_messages'] = "查看/发送消息";
$_lang['role_logout'] = "退出管理";
$_lang['role_help'] = "查看帮助文件";
$_lang['role_actionok'] = "查看完成行为";
$_lang['role_errors'] = "查看错误对话框";
$_lang['role_about'] = "查看关于页面";
$_lang['role_credits'] = "查看信用";
$_lang['role_change_password'] = "改变密码";
$_lang['role_save_password'] = "保存密码";
$_lang['role_view_docdata'] = "查看文档数据";
$_lang['role_create_doc'] = "建立新文档";
$_lang['role_edit_doc'] = "编辑一个文档";
$_lang['role_save_doc'] = "保存文档";
$_lang['role_delete_doc'] = "删除文档";
$_lang['role_cache_refresh'] = "清空站点cache";
$_lang['role_create_template'] = "建立新的站点模板";
$_lang['role_edit_template'] = "编辑站点模板";
$_lang['role_save_template'] = "保存模板";
$_lang['role_delete_template'] = "删除模板";
$_lang['role_create_snippet'] = "建立新的切片snippets";
$_lang['role_edit_snippet'] = "编辑切片snippets";
$_lang['role_save_snippet'] = "保存切片snippets";
$_lang['role_delete_snippet'] = "删除切片snippets";
$_lang['role_new_user'] = "建立新的用户";
$_lang['role_edit_user'] = "编辑用户";
$_lang['role_save_user'] = "保存用户";
$_lang['role_delete_user'] = "删除用户";
$_lang['role_access_persmissions'] = "访问许可";
$_lang['role_new_role'] = "建立新角色";
$_lang['role_edit_role'] = "编辑角色";
$_lang['role_save_role'] = "保存角色";
$_lang['role_delete_role'] = "删除角色";
$_lang['role_view_logs'] = "查看系统日志";
$_lang['role_edit_parser'] = "编辑站点剖析器parsers";
$_lang['role_save_parser'] = "保存剖析器parsers";
$_lang['role_edit_settings'] = "改变站点设置";
$_lang['role_file_manager'] = "使用文件管理器";
$_lang['confirm_unblock'] = "你确定解除用户锁定吗？";
$_lang['unblock_message'] = "保存这个用户数据后，这个拥护将不再被锁定了。";
$_lang['confirm_block'] = "你确定阻止这用户吗？";
$_lang['block_message'] = "保存数据后，这个用户将被阻止!";
$_lang['confirm_delete_user'] = "确定删除用户吗？";
$_lang['alert_delete_self'] = "你不可以删除你自己！";
$_lang['user_title'] = "建立/编辑用户";
$_lang['user_is_blocked'] = "这个用户被阻止了！";
$_lang['password_gen_method'] = "新密码方法";
$_lang['password_gen_gen'] = "让MODX生成新密码.";
$_lang['password_gen_specify'] = "让我指定密码:";
$_lang['password_method'] = "密码确认方法";
$_lang['password_method_email'] = "通过e-mail发送密码.";
$_lang['password_method_screen'] = "在屏幕上显示密码.";
$_lang['password_msg'] = " <b>%s</b>的新密码是 <b>%s</b>.";
$_lang['user_full_name'] = "全名";
$_lang['user_email'] = "E-mail地址";
$_lang['user_phone'] = "电话号码";
$_lang['user_mobile'] = "移动手机号码";
$_lang['user_role'] = "用户角色";
$_lang['user_logincount'] = "登陆次数";
$_lang['user_prevlogin'] = "最后登陆";
$_lang['user_failedlogincount'] = "失败的登陆";
$_lang['user_block'] = "阻止";
$_lang['user_edit_self_msg'] = "当你保存你的新信息后，你该退出然后再登陆，</b><br>同样，你该选择生成新的密码，新密码会通过EMAIL告诉你";
$_lang['confirm_delete_snippet'] = "确定要删除这个切片snippet?";
$_lang['snippet_title'] = "建立编辑切片snippet";
$_lang['snippet_msg'] = "你可以在这里添加编辑切片，记住, 切片snippets 是未加工的PHP代码，如果你希望它能输出内容到你模板指定的位置，你就需要这个切片返回一个值。";
$_lang['snippet_name'] = "切片名";
$_lang['snippet_desc'] = "描述";
$_lang['confirm_delete_template'] = "确定删除这个模板？";
$_lang['template_title'] = "建立编辑模板";
$_lang['template_msg'] = "你可以在这里添加编辑/模板，当你站点的CACHE没清空的时候，你是不可以看见COOKIE里的文件采取了这个新的模板，当然你可以用预览功能来看到他。";
$_lang['template_name'] = "模板名";
$_lang['template_desc'] = "描述";
$_lang['refresh_title'] = "刷新站点";
$_lang['refresh_published'] = "已经发布<b>%s</b>文档。";
$_lang['refresh_unpublished'] = "还有<b>%s</b>文档没发布。";
$_lang['refresh_cache'] = "Cache: 找到 <b>%s</b>文档在cache和被删除的cache里 <b>%d</b>。<p>当文档被访问者访问的时候，CACHE将被重新建立。"; // changed in 0.6
$_lang['about_title'] = "关于MODx";
$_lang['about_msg'] = "MODx is a <a href='http://modxcms.com/what-is-modx.html' target='_blank'>PHP Application Framework and Content Management System</a> licensed under the <a href=\"../assets/docs/license.txt\">GNU GPL</a>.";
$_lang['credits_htmlarea'] = "HTMLArea was created by mishoo. Currently it's the only cross-browser compatible editor available, and it's free too!"; // obsolete
$_lang['credits_adodb'] = "John Lim built this great database abstraction layer. Etomite uses only the MySQL bits, but it's lovely nonetheless!"; // obsolete
$_lang['credits_php'] = "MODx is powered by PHP!"; // obsolete
$_lang['credits_mysql'] = "MODx is powered by MySQL!"; // obsolete
$_lang['credits_webfx'] = "MODx uses a number of JavaScripts from Webfx. Great stuff!"; // obsolete
$_lang['credits_dTree'] = "MODx uses the excellent dTree script from DestroyDrop.com!"; // obsolete
$_lang['credits_shouts_title'] = "<b>Additional thanks to:</b>"; // obsolete
$_lang['credits_shouts_msg'] = "
<ul>
<li><b>Raymond Irving, Ryan Thrash</b> - Project Founders</li>
<li><b>Jason Coward, Victor Brilon, Adam Crownoble, Jeff Whitfield, Remon Sijrier</b> - Core Architecture &amp; Design</li>
</ul>"; // changed (this changes almost every release)  // obsolete
$_lang['messages_title'] = "消息";
$_lang['messages_inbox'] = "收件箱";
$_lang['messages_read_message'] = "阅读消息";
$_lang['messages_compose'] = "写消息";
$_lang['messages_not_allowed_to_read'] = "你不被允许读这个消息！";
$_lang['messages_system_user'] = "[System]";
$_lang['messages_reply'] = "答复";
$_lang['messages_forward'] = "转发";
$_lang['messages_from'] = "从";
$_lang['messages_sent'] = "Sent on";
$_lang['messages_subject'] = "主题";
$_lang['messages_no_messages'] = "没有消息。";
$_lang['messages_private'] = "私人的";
$_lang['messages_category'] = "分类";
$_lang['messages_send_to'] = "发送给";
$_lang['messages_user'] = "一个用户";
$_lang['messages_group'] = "一个组";
$_lang['messages_all'] = "每个人";
$_lang['messages_select_user'] = "选择一个用户";
$_lang['messages_select_group'] = "选择一个用户组";
$_lang['messages_message'] = "消息";
$_lang['messages_send'] = "发送";
$_lang['files_title'] = "文件浏览";
$_lang['files_files'] = "文件Files";
$_lang['files_filetype_notok'] = "不允许上传本类型文件!";
$_lang['files_upload_ok'] = "文件上传成功!";
$_lang['files_upload_copyfailed'] = "拷贝文件到指定目录失败 - 上传失败!";
$_lang['files_upload_error'] = "错误！";
$_lang['files_upload_error0'] = "你的上传出现了问题.";
$_lang['files_upload_error1'] = "上传文件太大！";
$_lang['files_upload_error2'] = "上传文件太大了！";
$_lang['files_upload_error3'] = "你上传的文件只发送了一部分。";
$_lang['files_upload_error4'] = "你必须选择一上传的文件";
$_lang['files_upload_error5'] = "上传发生了问题！";
$_lang['files_dir_listing'] = "目录列表:";
$_lang['files_access_denied'] = "拒绝访问！";
$_lang['files_top_level'] = "顶层目录";
$_lang['files_up_level'] = "上层目录";
$_lang['files_filename'] = "文件名";
$_lang['files_modified'] = "修改Modified";
$_lang['files_filesize'] = "文件大小";
$_lang['files_fileoptions'] = "选项";
$_lang['files_directories'] = "目录";
$_lang['files_data'] = "数据";
$_lang['files_dirwritable'] = "目录可写吗？";
$_lang['files_uploadfile'] = "上传文件";
$_lang['files_uploadfile_msg'] = "选择一上传的文件：";
$_lang['files_upload_inhibited_msg'] = "<b>传输性能被评比</b> - 请确定服务器是否可以上传或文件目录是否可写<br />";
$_lang['files_editfile'] = "编辑文件";
$_lang['files_viewfile'] = "查阅文件";
$_lang['files_file_upload'] = "上传文件";
$_lang['files_uploading'] = "上传 <b>%s</b> 到 <b>%s/</b><br />";
$_lang['files_file_type'] = "文件类型：";
$_lang['search'] = "搜索";
$_lang['search_criteria'] = "搜索标准";
$_lang['search_criteria_id'] = "按ID号搜索";
$_lang['search_criteria_title'] = "按标题搜索";
$_lang['search_criteria_content'] = "按内容搜索";
$_lang['search_criteria_id_msg'] = "输入文件ID号快速定位。";
$_lang['search_criteria_title_msg'] = "在文件标题中寻找有输入文字的文件.";
$_lang['search_criteria_content_msg'] = "寻找文件内容中有输入文字的文件";
$_lang['search_results'] = "搜索结果";
$_lang['search_empty'] = "搜索无结果。请放宽搜索范围！";
$_lang['search_results_returned_msg'] = "搜索结果返回 <b>%s</b> 文件，如果返回结果太多，请限制您的搜索条件， The two left-most columns will allow you to find the document in the tree or view the document. The two right most columns show, respectively, if a document has been deleted and what it's published status is.<p />";
$_lang['search_results_returned_id'] = "ID";
$_lang['search_results_returned_title'] = "标题";
$_lang['search_results_returned_desc'] = "描述";
$_lang['search_view_docdata'] = "查阅本项";
$_lang['search_item_deleted'] = "本项以被删除";
$_lang['help_title'] = "帮助";
$_lang['help_msg'] = "<p>你可以在这里寻求支持 <a href='http://modx.cn' target='_blank'>访问中文MODx网站</a>如果你有兴趣， <a href='mailto:modxcn@163.com?subject=MODx中文支持'>可以写信给我们</a>.";

/***************************************************************************/
// new in 0.5.2
/***************************************************************************/

$_lang['configcheck_images'] = "图象目录不可写";
$_lang['configcheck_images_msg'] = "图象目录不可写或不存在，图象管理功能将不能运行！";
$_lang['php_version_check'] = "MODx兼容PHP 4.0.3 和更高的版本，请升级PHP版本！";
$_lang['document_alias'] = "文件别名 alias";
$_lang['document_alias_help'] = "你可以设置一文件的别名，这样，你的文件就可以通过如此格式来访问?\n\nhttp://yourserver/alias\n\n当然，这只有在你设置了friendly URLs后才起作用！";

$_lang['user_management_title'] = "管理内容管理者";
$_lang['user_management_msg'] = "你可以在这里选择你需要编辑的内容管理者，内容管理者就是那些可以登陆到后台管理内容的人。";
$_lang['role_management_title'] = "角色管理";
$_lang['role_management_msg'] = "在这里你可以选择一需要编辑的角色。";
$_lang['manage_templates'] = "模板";
$_lang['template_management_msg'] = "选择你需要编辑的模板。";
$_lang['manage_plugins'] = "插件";
$_lang['snippet_management_msg'] = "选择你需要编辑的切片snippet";
$_lang['resource_management'] = "资源管理";
$_lang['parser_not_writable'] = "你想编辑的剖析器不可以写parser，请设置其为可写（通过FTP或服务器控制面板来实现）"; //obsolete
$_lang['server_protocol_title'] = "服务器类型：";
$_lang['server_protocol_message'] = "如果你的服务器是HTTP连接，请从这里设置：";
$_lang['server_protocol_http'] = "http";
$_lang['server_protocol_https'] = "https";
$_lang['settings_server'] = "服务器设置"; // deprecated
$_lang['settings_site'] = "站点设置";
$_lang['settings_furls'] = "Friendly URL设置";
$_lang['settings_users'] = "用户设置User settings"; //changed 0.6 - shortened
$_lang['settings_ui'] = "面板编辑器设置Interface & editor settings"; //changed 0.6 - added editor
$_lang['settings_reports'] = "报告设置"; // deprecated
$_lang['settings_editor'] = "编辑器设置"; // deprecated
$_lang['use_editor_title'] = "激活的编辑器:";
$_lang['use_editor_message'] = "你希望激活富文本编辑器吗？如果你习惯写HTML，你可以在这里关闭它！特别注意这个设置将应用到所有的文件和用户！";

/***************************************************************************/
// new in 0.5.3
/***************************************************************************/

$_lang['signupemail_title'] = "注册的e-mail:";
$_lang['signupemail_message'] = "你可以在这里设置好消息，当你的用户在这里注册的时候，MODX就会自动将此消息发送给他/她。内容中会包含他的用户名和密码。 <br /><b>注意：</b> 下面的站位符将被程序自动替代: <br /><br />[+sname+] - 你网站的名字 <br />[+saddr+] - 你网站的EMAIL地址， <br />[+surl+] - 你网站的地址，<br />[+uid+] - 用户登陆名或ID号，<br />[+pwd+] - 用户的密码， <br />[+ufn+] - 用户的全名。 <br /><br /><b>请在内容里保留 [+uid+] 和 [+pwd+],否则您的注册者将不会知道他的密码!</b>";
$_lang['emailsender_title'] = "邮件地址：";
$_lang['emailsender_message'] = "指定发送邮件的地址。";
$_lang['emailsubject_title'] = "邮件主题：";
$_lang['emailsubject_message'] = "在这里你可以指定注册成功后给用户发送的e-mail的标题.";
$_lang['stay'] = "继续编辑";
$_lang['saving'] = "保存中，稍侯...";
$_lang['charset_title'] = "编码:";
$_lang['charset_message'] = "请选择你的后台管理编码。注意MODX虽然测试了大量的编码类型，但是并非全部测试过了，对于大部分语言来说，编码设置是ido-8859-1。多语言设置是utf-8。简体中文是gb2312。";
$_lang['reset_failedlogins'] = "重设";
$_lang['untitled_document'] = "无标题文件";
$_lang['untitled_weblink'] = "无标题连接";
$_lang['layout_settings_title'] = "布局设置"; // deprecated
$_lang['layout_title'] = "管理布局:";
$_lang['layout_message'] = "利用这个选项来设置MODx的管理布局设置";
$_lang['layout_settings_1'] = "基本边栏设置 <br /><span class='comment'>导航在文件目录树的上方</span>";
$_lang['layout_settings_2'] = "顶部是菜单(只针对IE) <br /><span class='comment'>导航条在屏幕的上方。 本布局只在IE下起效</span>";
$_lang['loading_page'] = "请等待MODx加载页面...";

/***************************************************************************/
// new in 0.6
/***************************************************************************/

$_lang['filemanager_path_title'] = "文件管理路径:";
$_lang['filemanager_path_message'] = "IIS often does not populate the document_root setting properly, which is used by the file manager to determine what you can look at. If you're having problems using the file manager, make sure this path points to the root of your MODx installation.";
$_lang['password_gen_length'] = "密码至少要六个字符以上。";
$_lang['captcha_code'] = "安全码Security code";
$_lang["login_captcha_message"] = "管理员设置了安全模式，因此你需要输入安全码。\n\nI如果你看不清楚或看不见安全码，请点击代码本身来从新生成安全码?";
$_lang['captcha_title'] = "使用安全码：";
$_lang['captcha_message'] = "激活本选择，将加强网站的安全性，本功能生成的代码不可被黑客程序自动识别";
$_lang['lock_snippet'] = "锁住snippet";
$_lang['lock_snippet_msg'] = "只有管理员( ID号为 1)能编辑这个切片snippet。";
$_lang['snippet_locked_message'] = "本切片以锁住。";
$_lang['lock_template'] = "锁住模板";
$_lang['lock_template_msg'] = "只有管理员( ID号为 1)能编辑这个模板。";
$_lang['template_locked_message'] = "本模板被锁住。";
$_lang['administrator_role_message'] = "这个角色Role不可以被编辑或删除。";
$_lang['visitor_stats_online'] = "在线访问者";
$_lang['track_visitors_title'] = "访问日志";
$_lang['track_visitors_message'] = "访问日志能让你了解你的网站利用状态，当然这将会为网站增加一点负担。如果你不在乎网站的访问记录分析，你可以关闭这个功能，而让网站的访问速度的提到。";
$_lang['friendly_alias_title'] = "使用友好别名friendly aliases:";
$_lang['friendly_alias_message'] = "如果你在使用friendly URLs， 并且文件有别名， the alias will always have precedence over the friendly URL. By setting this option to 'yes', the friendly URL prefix and suffix will also be applied to the alias. For example, if your document with ID 1 has an alias of `introduction`, and you've set a prefix of `` and a suffix of `.html`, setting this option to `yes` will generate `introduction.html`. If there's no alias, MODx will generate `1.html` as link.";
$_lang['no_stats_message'] = "这里显示你站点的访问者状态，但是，当你在MODx配置里设置`访问日志日志Log visits`为`否no`的话这里将不收集任何信息。";
$_lang['welcome_visitor_stats'] = "这里显示了一些站点访问者的状态，如果你希望得到更多的信息，请访问`访问者状态Visitor Stats`页面.<br /><br />";
$_lang['page_impressions'] = "页面印象";
$_lang['visitors'] = "Unique visitors";
$_lang['visits'] = "Visits";
$_lang['all_time'] = "全部时间";
$_lang['this_month'] = "本月";
$_lang['today'] = "今天";
$_lang['document_visitor_stats'] = "<br /><br />本表格显示了本文档的访问者记录:<br />";
$_lang['resolve_hostnames_title'] = "解析服务器名";
$_lang['resolve_hostnames_message'] = "当访问者访问网站的时候，你希望MODX解析访问者服务器吗？解析服务器会导致额外的服务被加载，尽管访问者无法知道。";
$_lang['stats_title'] = "`%s` from %s to %s访问状态";
$_lang['stats_monthly_breakup'] = "每月统计";
$_lang['stats_daily_breakup'] = "每天统计";
$_lang['stats_hourly_breakup'] = "每小时统计";
$_lang['top_howmany_title'] = "Top how many";
$_lang['top_howmany_message'] = "When viewing reports, how large should the 'Top ...' lists be?";
$_lang['stats_top_pages'] = "最受关注的 %s 文档";
$_lang['document'] = "文档Document";
$_lang['rank'] = "评级";
$_lang['stats_top_referrers'] = "最受引用的 %s 页面";
$_lang['referrer'] = "引用者";
$_lang['stats_top_ua'] = "访问者最常用的 %s 浏览器";
$_lang['ua'] = "用户浏览器";
$_lang['stats_top_os'] = "访问者最常用的 %s 操作系统 ";
$_lang['os'] = "操作系统";
$_lang['visitors_used_this'] = "Visitors using this";
$_lang['visitors_came_from_here'] = "访问者来源地";
$_lang['no_results'] = "没有结果";
$_lang['no_online_users'] = "没有正浏览网站的访问者。";
$_lang['stats_top_hosts'] = "Top %s hosts";
$_lang['hostname'] = "Host name";
$_lang['unknown_referer'] = "Unknown/ Direct/ Bookmark";
$_lang['internal_referer'] = "站点内在关联";
$_lang['stats_top_entrypages'] = "最受关注的 %s 文档";
$_lang['visits_started_here'] = "从这里开始访问";
$_lang['no_logging_found'] = "MODx没发现登陆数据，这意味着你站点关闭了登陆或到现在还没有访问者。";
$_lang['visitor_online'] = "正浏览站点的访问者。";
$_lang['visitor_online_message'] = "这表格显示最近20分钟内活跃的用户(从 <b>%s</b>)。";
$_lang['access_permission_denied'] = "你没有访问这文档的权限。"; // changed 0.6 RC2
$_lang['warning'] = "警告！";
$_lang['documents'] = "文档";
$_lang['sysinfo_activity_message'] = "这个列表显示了最近有哪些文档被你的用户编辑过。";
$_lang['datechanged'] = "改动的时间";
$_lang['sysinfo_userid'] = "用户";
$_lang['id'] = "ID";
$_lang['settings_after_install'] = "作为全新安装，你将需要控制这些设置来符合你的要求，当你完成设置后，按'保存Save'更新数据库数据。<br /><br />";
$_lang['settings_misc'] = "杂项设置";
$_lang["udperms_allowroot_title"] = "允许的根目录:";
$_lang["udperms_allowroot_message"] = "你允许你的用户在站点根目录下建立文档和文件夹吗？ ";
$_lang['access_permission_parent_denied'] = "你不允许在这里建立文档！";
$_lang["new_htmlsnippet"] = "建立新块";
$_lang["edit_htmlsnippet"] = "编辑块";
$_lang['manage_htmlsnippets'] = "块管理";
$_lang['htmlsnippet_management_msg'] = "你可以在这里选择你需要编辑的块。";
$_lang['confirm_delete_htmlsnippet'] = "你确定要删除这个块？";
$_lang['htmlsnippet_title'] = "建立/编辑块";
$_lang['htmlsnippet_msg'] = "你可以在这里建立编辑块。记住，块是HTML代码组合，任何PHP代码将不会执行。";
$_lang['htmlsnippet_name'] = "块名";
$_lang['htmlsnippet_desc'] = "描述";
$_lang['lock_htmlsnippet'] = "锁住块的编辑";
$_lang['lock_htmlsnippet_msg'] = "只有超级管理员(ID 是1的角色)可以编辑这块";
$_lang['htmlsnippet_locked_message'] = "这块被锁定";
$_lang['access_permissions_user_groups'] = "用户组";
$_lang['access_permissions_document_groups'] = "文档组";
$_lang["access_permissions_links"] = "用户组和文档组的连接";
$_lang['rename'] = "重命名";
$_lang['remove'] = "移动";
$_lang['add'] = "添加组";
$_lang['access_permissions_introtext'] = "你可以在这里设置文档组和用户组来控制访问权的分配，要添加一个用户到用户组，你就编辑用户然后选择他归宿于那个组，要添加一个文档到一个用户组，就在你编辑文档的时候选择本文档从属哪个组就可以了。";
$_lang["access_permissions_add_user_group"] = "建立新的用户组";
$_lang["access_permissions_add_document_group"] = "建立新的文档组";
$_lang['no_groups_found'] = "没发现组。";
$_lang['access_permissions_user_group'] = "用户组：";
$_lang['access_permissions_user_group_access'] = "Document groups this user group has access to:";
$_lang['access_permissions_users_in_group'] = "<b>在本组的用户:</b> ";
$_lang['access_permissions_no_users_in_group'] = "没有。";
$_lang['access_permissions_documents_in_group'] = "<b>组里的文档：</b> ";
$_lang['access_permissions_no_documents_in_group'] = "没有。";
$_lang['access_permissions_docs_message'] = "你可以在这里设置文档归属于哪个文档组：";
$_lang['access_permissions_users_message'] = "你可以在这里设置用户该归属于哪个用户组：";
$_lang['access_permissions_users_tab'] = "你可以在这里查看建立了哪些用户组。你也可以建立/重命名/删除组。可以查看某个用户归属于哪些组。 要在组里添加/移除一个用户，就直接编辑用户。管理员 (users who have been assigned the role with ID 1)永远都拥有查看操作文件的权利，因此他不需要归属于任何组都可以。";
$_lang['access_permissions_documents_tab'] = "你可以在这里查看建立了哪些文档组。你也可以建立/重命名/删除组。查看哪些文档归属于哪个文档组，(把鼠标放在文件的ID上就可以看见它的名字). 要在阻力添加/移除一个用户，就直接编辑文档。";
$_lang['access_permissions_links_tab'] = "这里是我们分配用户组权限以对应到文档组的地方。 (比如编辑文档，建立子文档的权利) . 要连接一个文档组到一个用户组，选择一个组直接点'link 连接'就可以了。要移除他们的连接就直接点 '移除->Remove ->'就可以，这样连接就会立即失效。";
$_lang['access_permissions_off'] = "<span class='warning'>访问权控制没激活！</span> 这意味着这里的任何改动将不起任何作用，除非你激活了访问权控制。";
$_lang["settings_strip_image_paths_title"] = "重写图象路径？";
$_lang["settings_strip_image_paths_message"] = "开启这个选项后，MODx将把图象的路径由绝对定位改为相对的定位。这个功能将方便你的站点变迁移动或域名变换等，如果你不知道它代表什么意思的话，那你就选择'否No'。";
$_lang["uploadable_files_title"] = "允许上传文件的类型：";
$_lang["uploadable_files_message"] = "你可以在这里输入一串你允许上传文件类型的扩展名，注意每个扩展名用逗号隔开（注意是英文状态下的逗号）。";
$_lang["keywords"] = "关键词";
$_lang["keyword"] = "关键词";
$_lang["save_all_changes"] = "保存所有改动";
$_lang["new_keyword"] = "添加新的关键词:";
$_lang["confirm_delete_keywords"] = "确认删除关键词？";
$_lang["deselect_keywords"] = "清除关键词";
$_lang["sort_asc"] = "上升";
$_lang["sort_desc"] = "递减";
$_lang["sort_tree"] = "排列树";
$_lang["folder"] = "文件夹";
$_lang["pagetitle"] = "文档标题";
$_lang["defaulttemplate_title"] = "默认模板";
$_lang["defaulttemplate_message"] = "选择你建立的新文档所使用的默认模板。你也可以在文档编辑器里选择不同的模板，这个选择只是你为自己选择的一个预先设置的模板。";
$_lang["openurl_message"] = "你希望打开什么连接？";
$_lang["openurl"] = "打开外部网站";
$_lang["captcha_words_title"] = "机器机密码";
$_lang["captcha_words_message"] = "你可以在这里输入一串机器码以让抗机器功能工作，用逗号（注意是英文状态下的逗号）隔开每一字符串，你最多可以输入225个字符。";
$_lang["editing_file"] = "正在编辑的文档：";
$_lang["file_not_saved"] = "文件不能保存，请确认目的文件目录是否可写！";
$_lang["file_saved"] = "文件上传成功！";
$_lang['confirm_delete_file'] = "确定删除文件？\\n\\n这或许会让你的站点不稳定甚至不能工作，最好只删除你知道的文件！";
$_lang["deleting_file"] = "删除文件 `%s`: ";
$_lang["file_not_deleted"] = "失败！";
$_lang["file_deleted"] = "成功！";
$_lang["cache_files_deleted"] = "下面是被删除的文件：";
$_lang['export_site_html'] = "输出整个站点成为HTML文档";
$_lang['export_site_message'] = "使用这个功能，你可以将整个站点转化为HTML文件.注意：无论如何你都将失去MODX所提供的很多功能:<p /><ul><li>Page reads on the exported files will not be recorded.</li><li>切片功能将不能工作。</li><li>只有普通文件能够被输出，网站连接也不能输出。</li><li>如果你的文档中有切片使用了重定向功能，那文档也不能输出。</li><li>Depending on how you've written your documents, style sheets and images, the design of your site may be broken. To fix this, you can save/ move your exported files to the same directory where the main MODx index.php file is located.</li></ul><p />Please fill out the form and press 'Export' to start the export process. The files created will be saved in the location you specify, using, where possible, the document's aliases as filenames. While exporting your site, it's best to have the MODx configuration item 'Friendly aliases' set to 'yes'. Depending on the size of your site, the export may take a while.<p /><b>Any existing files will be overwritten by the new files if their names are identical!</b><p />";
$_lang['export_site_cacheable'] = "包含无cache功能的文档：";
$_lang['export_site_prefix'] = "文档前缀:";
$_lang['export_site_suffix'] = "文档后缀:";
$_lang['export_site_start'] = "开始输出";
$_lang['export_site_numberdocs'] = "发现<b> %s文档要输出...</b><p/>";
$_lang['export_site_exporting_document'] = "输出文档 <b>%s</b> of <b>%s</b><br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<small><i>%s</i>, id %s</small><br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
$_lang['export_site_success'] = "<span style='color:#009900'>成功！</span>";
$_lang['export_site_failed'] = "<span style='color:#990000'>失败！</span>";
$_lang['export_site_target_unwritable'] = "目标路径不可写。确认目标路径可写后再尝试！";
$_lang['export_site_time'] = "输出完成，一共花了 %s 秒。";
$_lang['export_site_maxtime'] = "最大输出时间：";
$_lang['export_site_maxtime_message'] = "你可以在这里设置MODx输出整个站点的最大时间，(覆盖PHP的设置).输入0表示时间无限制. 不过请注意，如果你输入0或时间过大，你的服务器或许会发生很奇怪的现象而不能访问，因此我们不建议设置0，最好把时间控制在你的服务器允许范围内。";
$_lang['page_data_contentType'] = "内容类型";
$_lang['page_data_contentType_help'] = "为文档选择内容类型。如果你不知道如何选择，请选择默认的text/html格式。";
$_lang["defaultcache_title"] = "默认Cache";
$_lang["defaultcache_message"] = "选择是（YES），就是每当新文档建立的时候，其都默认为允许高速缓存。";
$_lang["defaultpublish_title"] = "默认的发布选择";
$_lang["defaultpublish_message"] = "选择是（YES）的话，就是每当新文档建立的时候，其都默认为发布状态。";
$_lang["defaultsearch_title"] = "默认的搜索设置";
$_lang["defaultsearch_message"] = "选择是（YES）的话，就是每当新文档建立的时候，其都默认为可搜索状态。";
$_lang["which_editor_title"] = "选用编辑器:";
$_lang["which_editor_message"] = "你可以在这里选择你要使用哪一个编辑器。你可以在<a href='www.modx.cn'>MODX的中文网站</a>下载文本编辑器。";
$_lang["manage_modules"] = "管理模块";
$_lang["modules_already_installed"] = "安装模块";
$_lang["modules_install_new"] = "安装新模块";
$_lang["module_install_info"] = "由本模块安装的项目";
$_lang["type"] = "类型";
$_lang["alias"] = "别名";
$_lang["deleted"] = "删除";
$_lang["empty_folder"] = "文件夹为空";
$_lang["long_title"] = "长标题";
$_lang['document_long_title_help'] = "你可以在这里设置你文档的长标题。这方便了搜索引擎的工作，也添加了文档的描述。";
$_lang['search_criteria_longtitle'] = "按长标题搜索";
$_lang['search_criteria_longtitle_msg'] = "查询长标题里有所有输入查询字符的文档";
$_lang['empty_recycle_bin_empty'] = "没有需要清除的删除文件.";
$_lang['createdon'] = "建立日期";
$_lang['editedon'] = "编辑日期";
$_lang['sort_by'] = "按&nbsp;排序:";
$_lang['sort_dir'] = "按&nbsp;排序:";

/***************************************************************************/
// new to modx tech preview
/***************************************************************************/

$_lang["my_account"] = "我的帐号";
$_lang['logo_slogan'] = "简易而不简单 - \nMODx内容管理";

$_lang['sys_alert'] = "系统警报";
$_lang['confirm_delete_plugin'] = "确认删除这个插件？";
$_lang["plugin_event_msg"] = "选择插件所归属的时间.";
$_lang['plugin_title'] = "建立/编辑插件";
$_lang['plugin_msg'] = "在这里你可以建立/编辑插件，插件是一些PHP代码，当你选择的事件触发的时候，插件就被MODX系统调用.";
$_lang['plugin_name'] = "插件名";
$_lang['plugin_disabled'] = "禁用插件";
$_lang['plugin_desc'] = "描述";
$_lang['plugin_config'] = "插件配置";
$_lang['lock_plugin'] = "禁止插件被编辑";
$_lang['lock_plugin_msg'] = "只有管理员(Role ID 1) 可以编辑这个插件.";
$_lang['plugin_locked_message'] = "插件被锁定.";
$_lang['sunday'] = "星期天";
$_lang['monday'] = "星期一";
$_lang['tuesday'] = "星期而";
$_lang['wednesday'] = "星期三";
$_lang['thursday'] = "星期四";
$_lang['friday'] = "星期五";
$_lang['saturday'] = "星期六";
$_lang['comment'] = "评论";
$_lang['not_set'] = "没有设置";
$_lang['remove_date'] = "移动时间";
$_lang['select_date'] = "选择时间";
$_lang['no_date'] = "没有时间";
$_lang['settings_photo'] = "图片";
$_lang['user_photo'] = "用户图片";
$_lang['user_photo_message'] = "输入图象地址或上传一文件到服务器来作为用户图片。";
$_lang['user_fax'] = "传真";
$_lang['user_dob'] = "生日";
$_lang['user_country'] = "国家";
$_lang['user_state'] = "省份";
$_lang['user_zip'] = "Zip";
$_lang['user_gender'] = "性别";
$_lang['user_male'] = "男";
$_lang['user_female'] = "女";
$_lang['user_blockeduntil'] = "阻止到为止";
$_lang['user_blockedafter'] = "以后阻止";

$_lang['back'] = "后退";
$_lang['back_title'] = "加载预览页面";
$_lang['forward'] = "向前";
$_lang['reload'] = "重新加载";
$_lang['reload_title'] = "重新加载页面";

$_lang['websignupemail_title'] = "网站注册信:";
$_lang['websignupemail_message'] = "当用户注册后，系统将给他们发一封注册信，信的内容包含了他们的注册名字和密码。你可以在这里设置给他们的邮件的内容，<br /><b>注意:</b> 下面这些站位符将在发送邮件的时候被系统替换掉: <br /><br />[+sname+] - 站点名字, <br />[+saddr+] - 您的邮件地址, <br />[+surl+] - 你站点的连接, <br />[+uid+] - 用户的登陆名和ID号, <br />[+pwd+] - 用户的密码, <br />[+ufn+] -用户的全名. <br /><br /><b>在邮件里保留 [+uid+] 和 [+pwd+], 否则发送给他们的邮件里将没有登陆名和密码，他们或许就不知道他们的密码了!</b>";
$_lang['webpwdreminder_title'] = "网站提醒邮件:";
$_lang['webpwdreminder_message'] = "当用户请求重设密码的时候，系统将给他们发送一封电子邮件，在这里你可以指定这个电子邮件的内容. MODX将自动把密码和激活信息发送给用户。 <br /><b>注意:</b>当发送邮件的时候，系统将把下面的站位符替换掉: <br /><br />[+sname+] - 站点名字, <br />[+saddr+] - 您的邮件地址, <br />[+surl+] -你网站的 url, <br />[+uid+] - 用户的登陆名或id, <br />[+pwd+] - 用户的密码, <br />[+ufn+] - 用户的全名. <br /><br /><b>在邮件里请保留 [+uid+] 和 [+pwd+]，否则发送给他们的邮件里将没有登陆名和密码，他们或许就不知道他们的密码了!<</b>";

$_lang['click_to_context']="点击到关联菜单";
$_lang['snippet_execonsave']="保存切片后执行它.";
$_lang['snippets_availabe'] = "你可以插入页面的切片";
$_lang['use_alias_path_title'] = "使用友好别名路径:";
$_lang['use_alias_path_message'] = "Setting this option to 'yes' will display the full path to the document if the document has an alias. For example, if a document with an alias called 'child' is located inside a folder with an alias called 'parent', then the full alias path to the document will be displayed as '/parent/child.html'.<br /><b>NOTE: When setting this option to 'Yes' (turning on alias paths), you must reference things (such as images, css, javascripts, etc) using the absolute path:  e.g., '/assets/images' as oppose to 'assets/images'. By doing so you will prevent the browser (or web server) from appending the relative path to the alias path.</b>";
$_lang['scroll_up'] = "向上滚";
$_lang['scroll_dn'] = "向下滚";
$_lang['all_doc_groups'] = "所有的文档组(公共的)";
$_lang['value'] = "值";
$_lang['parameter'] = "参数";
$_lang['insert'] = "插入";
$_lang['insert_snippet'] = "把切片插到文档中";
$_lang['illegal_parent_child'] = "指定父本:\\n\\n这个文档是你选择的文档的子文档.";
$_lang['illegal_parent_self'] = "指定父本:\\n\\n选择的文档不能分配给他自己.";
$_lang['manager_theme'] ="管理者的风格文件:";
$_lang['manager_theme_message'] ="为管理者选择一个风格文件.";
$_lang['document_summary'] ="摘要 (介绍)";
$_lang['document_summary_help'] ="输入文档的大纲";
$_lang['close'] = "关闭";
$_lang['import_site_html'] = "从HTML输入站点";
$_lang['import_site_message'] = "使用这个功能能把整个HTML网站内容输入到数据库. 你需要把文件和文件夹先拷贝到 assets/import文件夹里.<p />填完表单后，请按'输入Import'就可以开始站点的输入了.输入的文件将保存你你选择的目录内,另外，文件的名字将作为文档的别名页面的标题将作为文档的标题.";
$_lang['import_parent_document'] = "上级文档（父本）:";
$_lang['import_site_start'] = "开始输入";
$_lang['import_files_found'] = "<b>有 %s 文档需要输入...</b><p/>";
$_lang['import_site_importing_document'] = "输入文件 <b>%s</b> ";
$_lang['import_site_skip'] = "<span style='color:#990000'>跳过!</span>";
$_lang['import_site_success'] = "<span style='color:#009900'>成功!</span>";
$_lang['import_site_failed'] = "<span style='color:#990000'>失败!</span>";
$_lang['import_site_time'] = "输入完成，一共花了 %s 秒.";
$_lang['import_site_maxtime'] = "最大输入时间:";
$_lang['import_site_maxtime_message'] = "你可以在这里设置输入过程占用的最大输入时间（覆盖PHP设置）输入0代表时间没限制，但是请注意：如果你输入的时间过大或输入0，你的服务器可能出现不可预料的情况，因此我们不建议你输入0，详细问题你可以咨询你的服务器技术人员.";
$_lang["duplicate"] = "复制";
$_lang["confirm_duplicate_record"] = "确认复制记录？";
$_lang["confirm_duplicate_document"] = "确认复制文档/文件夹\\n注意：子文件/夹也会被一同复制";
$_lang['tmplvar_inuse'] = "以下文档正在使用模板变量. 要继续的话请按删除（Delete），否则请按退出（Cancel）.";
$_lang['tmplvar_change_template_msg'] = "改变这个模板将让页面重新加载模板变量（TV）.所有没有保存的改动将丢失.\\n\\n 你确认要改变模板吗？";
$_lang['tmplvar_access_msg'] = "选择允许修改这个变量的内容和值的文档组";
$_lang['tmplvar_tmpl_access'] = "模板访问";
$_lang['tmplvar_tmpl_access_msg'] = "选择允许处理和利用变量的模板";
$_lang['tmplvars_novars'] = "没发现模板变量";
$_lang['tmplvars'] = "模板变量";
$_lang['tmplvars_management_msg'] = "在这里你可以管理你的文档额外的变量域.";
$_lang['tmplvars_reset_params'] ="重设参数";
$_lang['tmplvars_binding_msg'] ="这个域支持@命令捆绑来的数据";
$_lang['new_tmplvars'] = "新的模板变量";
$_lang['edit_tmplvars'] = "编辑模板变量";
$_lang['confirm_delete_tmplvars'] = "你确定要移除这些变量和那些存储好的变量吗？";
$_lang['tmplvars_msg'] = "在这里你可以添加/编辑模板变量。这模板变量对于选择的模板来说必须是激活的，只有这样他们才可以通过切片来存取。.";
$_lang['tmplvars_caption'] = "标题";
$_lang['tmplvars_name'] = "变量名";
$_lang['tmplvars_description'] = "描述";
$_lang['tmplvars_type'] = "输入类型";
$_lang['tmplvars_elements'] = "输入选项值";
$_lang['tmplvars_rank'] = "排列顺序";
$_lang['tmplvars_default'] = "默认值";
$_lang['tmplvars_widget'] = "窗口部件";
$_lang['tmplvars_widget_prop'] = "窗口部件设置";
$_lang['lock_tmplvars'] = "锁定变量使其不得编辑";
$_lang['lock_tmplvars_msg'] = "只有管理员(Role ID 1)才可以编辑这个变量 .";
$_lang['tmplvars_locked_message'] = "变量已被锁住.";
$_lang['document_setting'] = "文档设置";
$_lang["bk_manager"] ="备份管理";
$_lang["upload_maxsize_title"] ="最大上传大小";
$_lang["upload_maxsize_message"] ="输入通过文件管理上传文件的大小的最大值. 大小按 byte计算. <b>注意: 大文件会占用太多的传输时间！</b>";
$_lang["file_folder_deleted"] ="成功删除文件夹!";
$_lang["file_folder_not_deleted"] ="不能删除文件夹，请先确认文件夹是否是空的！";
$_lang["confirm_unzip_file"] ="你希望解压文件吗？\\n\\n现在存在的文件会被覆盖。";
$_lang["file_unzip"] ="解压成功！";
$_lang["file_unzip_fail"] ="解压失败！";
$_lang["file_folder_created"] ="建立文件夹成功！";
$_lang["file_folder_not_created"] ="不能建立文件夹";
$_lang["file_download_file"] = "下载文件";
$_lang["file_download_unzip"] = "解压文件";
$_lang["login_homepage"] = "登陆页面";
$_lang["login_homepage_message"] = "输入当他/她登陆网站后需要呈现给他/她的页面的ID号 。<b>注意：你需要保证你输入的ID号是有效的，并且这个ID代表的文档是可以被他/她访问的!</b>";
$_lang["login_allowed_ip"] = "允许的IP地址";
$_lang["login_allowed_ip_message"] = "输入允许这个用户登陆时候的IP地址， <b>注意：用逗号来分开多个IP地址 (,)</b>";
$_lang["login_allowed_days"] = "允许访问的日期";
$_lang["login_allowed_days_message"] = "选择允许用户登陆的日期.";
$_lang["settings_general"] = "一般";
$_lang["settings_keywords"] = "关键字";
$_lang["settings_page_settings"] = "页面设置";
$_lang["settings_snippets"] = "切片";
$_lang["settings_properties"] = "属性";
$_lang["settings_config"] = "配置";
$_lang["settings_events"] = "系统事件";
$_lang["settings_templvars"] = "模板变量";
$_lang["unauthorizedpage_title"] = "未授权的页面:";
$_lang["unauthorizedpage_message"] = "输入文档ID号，这个文档是发送给那些访问到受保护文件或需要授权才能访问的文件的人，让他们明白他们没有权利访问这些文件。 <b>注意: 注意：你需要保证你输入的ID号是有效的，并且这个ID代表的文档是可以被他/她访问的!!</b>";
$_lang["template_reset_all"] = "设置所有的文档使用默认的模板";
$_lang["template_reset_specific"] = "只设置 '%s' 页面";
$_lang["show_preview"] = "显示预览窗口";
$_lang['web_access_permissions_user_group'] = "网站用户组:";
$_lang['web_access_permissions_user_groups'] = "网站用户组";
$_lang['web_access_permissions'] = "网站访问控制";
$_lang['web_user_title'] = "建立/编辑网站用户";
$_lang["snippet_properties"] = "默认属性";

$_lang["add_folder"] = "新文件夹";
$_lang["new_web_user"] = "新网站用户";
$_lang["new_plugin"] = "新插件";
$_lang["edit_plugin"] = "编辑插件";
$_lang["create_folder_here"] = "在这里建立文件夹";
$_lang["preview_msg"] = "这个是你最后一次改动的预览. 点击这里 <a href='javascript://' onclick=\"saveRefreshPreview();\"><img src='media/images/icons/save.gif' align='absmiddle' height='12'> 保存刷新</a>你现在的页面";
$_lang['role_plugin_management'] = "插件管理";
$_lang['role_create_plugin'] = "建立新的插件";
$_lang['role_edit_plugin'] = "编辑插件";
$_lang['role_save_plugin'] = "保存插件";
$_lang['role_delete_plugin'] = "删除插件";
$_lang['role_bk_manager'] = "使用备份管理";
$_lang['web_user_management_title'] = "管理网站用户";
$_lang['web_user_management_msg'] = "在这里你可以选择那些你想编辑的网站用户。网站用户就是那些允许登陆到网站的用户";
$_lang['plugin_management_msg'] = "在这里你可以选择你想编辑的切片";
$_lang['manage_snippets'] = "切片";
$_lang['stay_new'] = "添加另一个";

$_lang['layout_settings_3'] = "三行布局 <br /><span class='comment'>导航条在文档的左边。</span>";

$_lang["lock_msg"]="%s 正在编辑 %s. 等他结束后再尝试.";
$_lang["lock_settings_msg"]="%s 在编辑这些设置.  等他结束后再尝试.";
$_lang['module_management'] = "管理模块";
$_lang['confirm_delete_module'] = "你希望删除模块吗？";
$_lang['settings_dependencies'] = "依赖性";
$_lang['module_title'] = "建立/编辑模块";
$_lang['module_msg'] = "在这里你可以建立/编辑模块. 模块就是一些资源的集合 (比如插件，切片等等).";
$_lang['module_name'] = "模块名";
$_lang['module_disabled'] = "禁用模块";
$_lang['module_desc'] = "描述";
$_lang['module_config'] = "模块配置";
$_lang['lock_module'] = "锁定模块的编辑";
$_lang['lock_module_msg'] = "只有管理员 (Role ID 1)才可以编辑这个模块.";
$_lang['module_management_msg'] = "在这里你可以选择你希望运行或编辑的模块，要运行模块的话，就在他的图标上点击，如果你要修改编辑的话，就点击他的名字。";
$_lang['new_module'] = "新模块";
$_lang['edit_module'] = "编辑模块";
$_lang['module_viewdepend_msg'] = "在这里你可以查看分配给模块的资源，在'倚赖性'上点击就可以修改它的依赖性了。";
$_lang['module_depend_msg'] = "利用右边的按钮就可以添加/移除模块的依赖关系了.";
$_lang['confirm_load_depends'] = "你确定在不保存所做的修改就加载依赖性管理页面吗？";
$_lang['enable_resource'] = "激活资源文件.";
$_lang['resource'] = "资源";
$_lang['wrap_lines'] = "Wrap lines";
$_lang['icon'] = "图标";
$_lang["manage_depends"] = "依赖性管理";
$_lang["group_access_permissions"] = "用户组访问";
$_lang["module_group_access_msg"] = "选择可以在后台执行模块的用户组.";
$_lang["all_usr_groups"] = "全部用户组 (公共的)";
$_lang["resource_name"] = "资源名";
$_lang["name"] = "名字";
$_lang["description"] = "描述";
$_lang["locked"] = "锁定的";
$_lang["disabled"] = "禁用的";
$_lang['run_module'] = "运行模块";
$_lang['module_edit_click_title'] = "点这里运行模块";
$_lang['confirm_delete_record'] = "你确认要删除选择的记录？";
$_lang['module_resource_title'] = "模块依赖性";
$_lang['module_resource_msg'] = "在这里你可以添加/移除模块所依赖的资源，要添加一个新的资源就点击下面的添加按钮。";
$_lang['add_doc'] = "添加文档";
$_lang['add_snippet'] = "添加切片";
$_lang['add_chunk'] = "添加块元素";
$_lang['add_plugin'] = "添加插件";
$_lang['add_template'] = "添加模板";
$_lang['add_tv'] = "添加模板变量";
$_lang['resource_selector_title'] = "资源选择器";
$_lang['resource_selector_msg'] = "选择下面的资源然后按'插入Insert'键.";
$_lang['search'] = "搜索";
$_lang['reset'] = "重设";
$_lang['snippet'] = "切片";
$_lang['plugin'] = "插件";
$_lang['template'] ="模板";
$_lang['chunk']="块元素";
$_lang['tv']="模板变量";
$_lang['category']="分类";
$_lang['enable_sharedparams']="激活参数分享";
$_lang['enable_sharedparams_msg']="<b>注意:</b> 上面的全局唯一性ID(GUID)将用来标志这个模块和分享变量.  GUID 也是用来连接模块和插件/切片的参数访问控制的唯一标志. ";
$_lang['guid']="GUID";
$_lang['update_params']="更新参数显示";
$_lang['import_params']="输入模块的共享参数";
$_lang['import_params_msg']="你可通过从上面的下拉菜单中选择模块的名字来输入他的参数或设置. <b>注意:</b>为了模块能显示在菜单中，这个插件或切片必须是在模块的以来性列表中，并且模块的参数分享也必须激活. ";
$_lang['list_mode']="关闭打开列表模式 - 用来显示所有在格子中的数据.";
$_lang['role_module_management'] = "模块管理";
$_lang['role_new_module'] = "建立模块";
$_lang['role_edit_module'] = "编辑模块";
$_lang['role_save_module'] = "保存模块";
$_lang['role_delete_module'] = "删除模块";
$_lang['role_run_module'] = "运行模块";
$_lang['role_eventlog_management'] = "事件日志管理";
$_lang['role_view_eventlog'] = "查看事件日志";
$_lang['role_delete_eventlog'] = "删除事件日志";
$_lang['view_log'] = "查看日志";
$_lang['date'] = "日期";
$_lang['source'] = "来源";
$_lang['event_id'] = "事件Id";
$_lang['user'] = "用户";
$_lang['eventlog'] = "事件日志";
$_lang['eventlog_viewer'] = "事件日志查看器";
$_lang['eventlog_msg'] = "事件日志是用来显示由系统自动收集产生信息，警告，错误消息。这些'来源行'显示了这些消息来自何方.";
$_lang['clear_log'] = "清除日志";
$_lang['click_to_view_details'] = "点击这里查看细节";
$_lang['information']="信息";
$_lang['error']="错误";
$_lang['confirm_delete_eventlog'] = "你确认要删除事件日志？";

$_lang['editor_css_path_title'] = "CSS文件路径:";
$_lang['editor_css_path_message'] = "输入编辑器的CSS文件的路径。 最好的方法就是输入从根目录开始的路径, 比如: /assets/site/style.css. 如果你不希望加载编辑器的CSS, 请留空.";
$_lang['rb_title'] = "激活资源浏览器:";
$_lang['rb_message'] = "Select yes to enable the resource browser. This will allow your users to browse and upload resources such as images, flash and media files on the server.";
$_lang['rb_base_dir_title'] = "资源路径:";
$_lang['rb_base_dir_message'] = "Enter the physical path to the resource directory. This setting is usually automatically generated. If you're using IIS, however, MODx may not be able to work the path out on it's own, causing the Resource Browser to show an error. In that case, you can enter the path to the images directory here (the path as you'd see it in Windows Explorer). <b>NOTE:</b> The resource directory must contain the subfolders images, files, flash and media in order for the resource browser to function correctly.";
$_lang['rb_base_url_title'] = "资源的URL:";
$_lang['rb_base_url_message'] = "Enter the virtual path to resource directory. This setting is usually automatically generated. If you're using IIS, however, MODx may not be able to work the URL out on it's own, causing the Resource Browser to show an error. In that case, you can enter the URL to the images directory here (the URL as you'd enter it on Internet Explorer).";

$_lang['noresults_title'] = "结果数量:";
$_lang['noresults_message'] = "Enter the number of results to show in the data grid when viewing listings and search results.";
$_lang['default'] = "默认";
$_lang['basic'] = "基本";
$_lang['standard'] = "标准";
// $_lang['advanced'] = "Advanced"; // see line 150
$_lang['custom'] = "定制";
$_lang['you_got_mail'] = "你有邮件";
$_lang['email'] = "Email";
$_lang['click_to_edit_title'] = "点击这里编辑记录";
$_lang['manager_permissions'] = "管理者许可";
$_lang['web_permissions'] = "网站许可";
$_lang['export_site'] = "输出站点";
$_lang['import_site'] = "输入站点";
$_lang['mgr_access_permissions'] = "管理者访问许可";
$_lang['metatags'] = "META标签";
$_lang['meta_keywords'] = "META关键词";
$_lang['metatags_and_keywords'] = "META标签和关键词";
$_lang['metatag_intro'] = "On this page you can delete, create or edit META tags. To link META tags to documents, click on the <u>META Keywords</u> tab when editing the document, and select the desired META tags and keywords. To add a new tag enter the name and value and click the 'Add tag' button. To edit the tag click on the name of the tag from within the data grid.";
$_lang['keywords_intro'] = "To edit a keyword, simply type in the new keyword in the text field next to the keyword you wish to change. To delete a keyword, check the 'delete' box for that keyword. If you check the keyword's delete box, and also change it's name, it will be deleted, and the renaming of the keyword will not take place!";
$_lang['manage_metatags'] = "管理META标签和关键词";
$_lang['metatag_message'] = "在这个页面你可以管理你站点的META标签和关键词 ";
$_lang['tag'] = "标签";
$_lang['require_tagname'] = "需要一标签名";
$_lang['require_tagvalue'] = "需要一标签值";
$_lang['confirm_delete_tags'] = "你确认要删除选择的META 标签?";
$_lang['no_keywords_found'] = "这里通常没有关键词.";
$_lang['add_tag'] = "添加标签";
$_lang['save_tag'] = "保存标签";
$_lang['delete_tags'] = "删除标签";
$_lang['document_metatag_help'] = "Select the META tags or keywords you wish to assign to this document. Hold down the CTRL key to select multiple keywords or meta tags.";
$_lang["deselect_metatags"] = "清除META标签";
$_lang['no_records_found'] = "没有记录.";
$_lang['role_edit_doc_metatags'] = "编辑文档的META标签和关键词";
$_lang['role_manage_metatags'] = "管理站点的META标签和关键词";
$_lang['role_web_user_management'] = "网站用户管理";
$_lang['role_new_web_user'] = "建立新的网站用户";
$_lang['role_edit_web_user'] = "编辑新的网站用户";
$_lang['role_save_web_user'] = "保存网站用户";
$_lang['role_delete_web_user'] = "删除网站用户";
$_lang['role_web_access_persmissions'] = "网站访问许可";
$_lang['document_opt_trackvisit_help'] = "Log each visitor's visit to this page";
$_lang['document_opt_menu_title'] = "菜单标题";
$_lang['document_opt_menu_title_help'] = "Menu title is a field you can use to display a short title for the document inside your menu snippet(s) or modules.";
$_lang['document_opt_contentdispo'] = "内容部署";
$_lang['document_opt_contentdispo_help'] = "Use the content disposition field to specify how this document will be handled by the web browser. For file downloads select the Attachment option.";
$_lang['duplicate_alias_found'] = "Document '%s' is already using the alias '%s'. Please enter a unique alias.";
$_lang['page_data_web_access'] = "网站访问";
$_lang['page_data_mgr_access'] = "管理者访问";
$_lang['automatic_alias_title'] = "自动生成别名:";
$_lang['automatic_alias_message'] = "Select 'yes' to have the system automatically generate an alias based on the document's page title when saving.";
$_lang['duplicate_alias_title'] = "允许复制别名:";
$_lang['duplicate_alias_message'] = "Here you can select 'yes' to allow duplicate aliases to be saved. <b>NOTE: This option should be used with 'Friendly alias path' option set to 'Yes' in order to avoid problems when referencing a document.</b>";
$_lang['public'] = "公共的";
$_lang['private'] = "私有的";
$_lang['siteunavailable_page_title'] = "关闭站点的页面:";
$_lang['siteunavailable_page_message'] = "输入关闭站点后需要显示给访问者的页面ID. <b>注意必须保证你提供的ID号所属文档是的确存在的，并且已经发布，访问者有权访问!</b>";
$_lang['custom_contenttype_title'] = "定制内容类型:";
$_lang['custom_contenttype_message'] = "Here you can add custom content types to be used in your documents. To add a new entry, enter the content type in the text box then click the 'Add' button.";
$_lang['inline'] = "一行内";
$_lang['attachment'] = "附件";
$_lang['add'] = "添加";
$_lang['change_name'] = "改变名字";
$_lang['confirm_name_change'] = "Changing the user name can affect other applications that are linked to the Content Manager. \\n\\n Are you sure you want to change this user name?";

$_lang['alert'] = "配制";
$_lang['online'] = "在线";
$_lang['info'] = "信息";
$_lang['statistics'] = "统计";
$_lang['recent_docs'] = "最近文档";
$_lang['credits_everaldo'] = "MODx uses a number of icons from Everaldo's crystal icon set. Great icons!";
$_lang['security'] = "保密机密";
$_lang['web_users'] = "网站用户";
$_lang['backup'] = "备份";
$_lang['modules'] = "模块";
$_lang['layout_settings_4'] = "Collapsible top bar layout<br /><span class='comment'>Collapsible navigation bar to the top the document tree and main window.</span>";
$_lang['document_opt_show_menu'] = "显示在菜单中";
$_lang['document_opt_show_menu_help'] = "Select this option to show document inside a web menu. Please note that some Menu Builders might choose to ignore this option.";
$_lang['months'] = array("Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec");
$_lang['days'] = array("Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat");

$_lang['optimize_table'] = "点击这里优化表格";
$_lang['truncate_table'] = "点击这里切短表格";

// TP3.3
$_lang['mgr_login_start'] = "管理者的登陆启动页面";
$_lang["mgr_login_start_message"] = "Enter the ID of the document you want to send the user to after he/she has logged into the manager. <b>NOTE: make sure the ID you've enter belongs to an existing document, and that it has been published and is accessible by this user!</b>";
$_lang['allow_mgr_access'] = "管理员的界面访问";
$_lang['allow_mgr_access_message'] = "Select this option to enable or disable access to the manager interface. <b>NOTE: If this option is set to no then the user will be redirected the the Manager Login Startup or Site Start web page.</b>";
$_lang['logout_message'] = "你现在已经登陆了站点管理. 如果你要退出，请按'退出Logout'. <p />要到管理首页请按 '主页Home' .";
$_lang['layout_settings_5'] = "上方菜单条布局<br /><span class='comment'>导航条在文档树目录和主要内容的上方.</span>";
$_lang['snippet_code'] = "切片代码 (php)";
$_lang['plugin_code'] = "插件代码 (php)";
$_lang['module_code'] = "模块代码 (php)";
$_lang['chunk_code'] = "块元素代码 (html)";
$_lang['template_code'] = "模板代码 (html)";
$_lang['switch_to_rte'] = "转换到富文本编辑器";

/***************************************************************************/
// new in 0.9.1
/***************************************************************************/
$_lang['QE_lang'] = 'simple_chinese';
$_lang['QE_xml_lang'] = 'simple_chinese';
$_lang['QE_charset'] = 'gb2312';
$_lang['QE_title'] = "QuickEdit"; // please change only if it violates local trademarks
$_lang['QE_show_links'] = "显示连接";
$_lang['QE_hide_links'] = "隐藏连接";
$_lang['QE_someone_editing'] = "其他人正编辑这个文档";
$_lang['QE_cant_find_content'] = "没有找到可以编辑的文档";
$_lang['QE_description'] = "从前台编辑文档";

$_lang['system_email_signup'] = "Hello [+uid+] \n\n这是你的内容管理登陆细节 [+sname+] :\n\n用户名: [+uid+]\n密码: [+pwd+]\n\n你可以登陆内容管理 ([+surl+]),来改变你的密码.\n\n谢谢,\n网站管理员";
$_lang['system_email_websignup'] = "Hello [+uid+] \n\n这是你的登陆细节 [+sname+]:\n\n用户名: [+uid+]\n密码: [+pwd+]\n\n你可以点这个登陆 [+sname+] ([+surl+]), 在那里也可以改变你的密码.\n\n谢谢,\n网站管理员";
$_lang['system_email_webreminder'] = "Hello [+uid+]\n\n点下面的连接来激活你的新密码:\n\n[+surl+]\n\n如果成功，你就可以用以下的密码来登陆:\n\n密码:[+pwd+]\n\n如果你不需要这个EMAIL，请忽略它.\n\n谢谢,\n网站管理员";

$_lang['role_view_unpublished'] = "查看未发布文档";
$_lang['duplicate_document'] = "复制文档Duplicate Document";
?>