...
class DocumentParser {
	...
	function sendUnauthorizedPage() {
		...
		$this->sendRedirect($this->makeUrl($this->config['unauthorized_page']),1);
	}
	...
	function getSettings() {
		...
		if(file_exists($base_path."/assets/cache/siteCache.idx")) {
			include_once $base_path."/assets/cache/siteCache.idx";
		} else {
		...
		if($id=$this->getLoginUserID()){
			if ($this->getLoginUserType()=='web') $query = $this->getFullTableName("web_user_settings")." WHERE webuser='$id'";
	...
	function checkCache($id) {
		$cacheFile = "assets/cache/docid_".$id.".pageCache";
	...
	function checkPublishStatus(){
		include $this->config["base_path"]."/assets/cache/sitePublishing.idx";
		...
		if($cacheRefreshTime<=$timeNow && $cacheRefreshTime!=0) {
			...
			$basepath = $this->config["base_path"]."/assets/cache";
			$fp = @fopen($basepath."/sitePublishing.idx","wb");
			if($fp) {
				@flock($fp, LOCK_EX);
				$len = strlen($data);
	...
	function mergeHTMLSnippetsContent($content) {
		...
		for($i=0; $i<$settingsCount; $i++) {
			if(isset($this->chunkCache[$matches[1][$i]])) {
				...
				$sql = "SELECT * FROM ".$this->getFullTableName("site_htmlsnippets")." WHERE ".$this->getFullTableName("site_htmlsnippets").".name='".$matches[1][$i]."';";
	...
	function evalSnippets($documentSource) {
		...
		for($i=0;$i<$nrSnippetsToGet;$i++) {	// Raymond: Mod for Snippet props
			if(isset($this->snippetCache[$matches[1][$i]])) {			
				...
				$sql = "SELECT * FROM ".$this->getFullTableName("site_snippets")." WHERE ".$this->getFullTableName("site_snippets").".name='".$matches[1][$i]."';";
				$result = $this->dbQuery($sql);
				if($this->recordCount($result)==1) {
					$row = $this->fetchRow($result);					
					$snippets[$i]['name'] = $row['name'];
					$snippets[$i]['snippet'] = $this->snippetCache[$row['name']] = base64_encode($row['snippet']);
					$snippets[$i]['properties'] = $this->snippetCache[$row['name']."Props"] = base64_encode($row['properties']);
				} else {
					$snippets[$i]['name'] = $matches[1][$i];
					$snippets[$i]['snippet'] = $this->snippetCache[$matches[1][$i]] = base64_encode("return false;");
	...
	function executeParser() {
		...
		if($this->documentContent!="") {
			...
			$tbldgn = $this->getFullTableName("documentgroup_names");
			...
			$sql = "SELECT DISTINCT sc.* 
					...
					AND (1='".$_SESSION['role']."' OR ".(IN_PARSER_MODE ? "NOT(dgn.private_webgroup<=>1)":"NOT(dgn.private_memgroup<=>1)").(!$docgrp ? "":" OR dg.document_group IN ($docgrp)").");";
			$result = $this->dbQuery($sql);
			$rowCount = $this->recordCount($result);
			if($rowCount<1) {
				if ($this->config['unauthorized_page']) {
					// check if file is not public
					$secrs = $this->dbQuery("SELECT id FROM $tbldg WHERE document = '".$this->documentIdentifier."' LIMIT 1;");
			...
			// load TVs - Orig by Apodigm - Docvars
			...
			$sql.= "LEFT JOIN ".$tbn."site_tmplvar_contentvalues tvc ON tvc.tmplvarid=tv.id AND tvc.contentid = '".$this->documentIdentifier."' ";			$sql.= "WHERE tvtpl.templateid = ".$this->documentObject['template'];
			$rs = $this->dbQuery($sql);
			...
			// Insert Startup jscripts into template - template must have a <head> tag
			if ($js = $this->getRegisteredClientStartupScripts()){
				$documentSource = preg_replace("/(<head>)/i", "\\1".$js, $documentSource);
			}

			// Insert jscripts into template - template must have a </body> tag
			if ($js = $this->getRegisteredClientScripts()){
				$documentSource = preg_replace("/(<\/body>)/i", $js."\\1", $documentSource);
			}
...
/***************************************************************************************/
	...
	function getDocumentChildren($parentid=0, $published=1, $deleted=0, $fields="*", $where='', $sort="menuindex", $dir="ASC", $limit="") {
		...
		$sql = "SELECT DISTINCT $fields FROM $tblsc sc
				...
				AND (1='".$_SESSION['role']."' OR ".(IN_PARSER_MODE ? "NOT(dgn.private_webgroup<=>1)":"NOT(dgn.private_memgroup<=>1)").(!$docgrp ? "":" OR dg.document_group IN ($docgrp)").") ";
		if ($sort) $sql.=" ORDER BY $sort $dir $limit ";
		$result = $this->dbQuery($sql);
		$resourceArray = array();
		for($i=0;$i<@$this->recordCount($result);$i++)  {
			array_push($resourceArray,@$this->fetchRow($result));
	...
	function runSnippet($snippetName, $params=array()) {
		if(isset($this->snippetCache[$snippetName])) {
			...
			$sql = "SELECT * FROM ".$this->getFullTableName("site_snippets")." WHERE ".$this->getFullTableName("site_snippets").".name='".$snippetName."';";
			$result = $this->dbQuery($sql);
			if(!$this->recordCount($result)==1) {
				$row = $this->fetchRow($result);					
				$snippet = $this->snippetCache[$row['name']] = base64_encode($row['snippet']);
				$properties = $this->snippetCache[$row['name']."Props"] = base64_encode($row['properties']);
			} else {
				$snippet = $this->snippetCache[$snippetName] = base64_encode("return false;");
				$properties = '';
			}
		}
		...
		$parameters = array();
		if(!empty($properties)) {
			$tempSnippetParams = explode("&",base64_decode($properties));
			for($x=0; $x<count($tempSnippetParams); $x++) {
				$pTmp = explode("=", $tempSnippetParams[$x]);
	...
	function getUserDocGroups($resolveIds = false){	
		$dg = (isset($_SESSION['docgroups']))? $_SESSION['docgroups']:'';
	...
	function isMemberOfWebGroup($groupNames=array()){
		...
		$ds = $this->dbQuery("SELECT wgn.name 
			FROM $tbl wgn 
			INNER JOIN $tbl2 wg ON wg.webgroup=wgn.id AND wg.webuser='".$_SESSION['internalKey']."' 
			WHERE wgn.name IN ('".implode("','",$groupNames)."')");
		if ($this->recordCount($ds)>0) return true;
		else return false;		
	}
	...
	function regClientStartupScript($src){		
		...
		if (strpos($src,"<script")!==false) $this->sjscripts[count($sjscripts)] = $src;
	...
	function regClientScript($src){		
		if ($this->loadedjscripts[$src]) return '';
		$this->loadedjscripts[$src] = true;
		if (strpos(src,"<script")===0) $this->jscripts[count($jscripts)] = src;
	...
	function invokeEvent($evtName,$extParams = array()){
		...
		if(count($el)>0) for ($i=0;$i<count($el);$i++) { // start for loop
			...
			if(isset($this->pluginCache[$pluginName])) {			
				...
				if(!$this->recordCount($result)==1) {
					$row = $this->fetchRow($result);					
					$pluginCode = $this->pluginCache[$row['name']] = base64_encode($row['plugincode']);
					$pluginProperties = $this->pluginCache[$row['name']."Props"] = base64_encode($row['properties']);
				} else {
					$pluginCode = $this->snippetCache[$pluginName] = base64_encode("return false;");
					$pluginProperties = '';
				}
			}
			...
			// load default params/properties 
			$parameter = array();
			if(!empty($pluginProperties)) {
				$tmpParams = explode("&",base64_decode($pluginProperties));
				for($x=0; $x<count($tmpParams); $x++) {
					$pTmp = explode("=", $tmpParams[$x]);
					$pvTmp = explode(";", trim($pTmp[1]));
					if ($pvTmp[1]=='list' && $pvTmp[3]!="") $parameter[$parameterTemp[0]] = $pvTmp[3]; //list default
...
class SystemEvent {
	...
	function alert($msg) {
		...
		if (is_array($SystemAlertMsgQueque)) {
			$SystemAlertMsgQueque[] = "<div><b>".$this->activePlugin."</b> - <span style='color:maroon;'>".$this->name."</span></div><div style='margin-left:10px;margin-top:3px;'>$msg</div>";
		}
	}
