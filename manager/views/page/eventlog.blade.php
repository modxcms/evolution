...
@section('content')
    ...
    $cm->addItem(ManagerTheme::getLexicon('view_log'), "js:menuAction(1)", $_style['actions_preview']);
    $cm->addSeparator();
    $cm->addItem(ManagerTheme::getLexicon('delete'), "js:menuAction(2)", $_style['actions_delete'], (!$modx->hasPermission('delete_eventlog') ? 1 : 0));
    echo $cm->render();
    ?>
    @push('scripts.top')
    ...
    <form name="resource" method="post">
        ...
        <h1>
            <i class="{{ $_style['actions_triangle'] }}"></i>{{ ManagerTheme::getLexicon('eventlog_viewer') }}<i class="{{ $_style['icons_tooltip'] }} help"></i>
        </h1>
                ...
                <div class="searchbar form-group">
                    <div class="input-group">
                        <div class="input-group-btn">
                            <a class="btn btn-danger btn-sm" href="index.php?a=116&cls=1"><i class="{{ $_style['actions_delete'] }}"></i> {{ ManagerTheme::getLexicon('clear_log') }}</a>
                        </div>
                        ...
                        <div class="input-group-btn">
                            <a class="btn btn-secondary btn-sm" href="javascript:;" title="{{ ManagerTheme::getLexicon('search') }}" onclick="searchResource();return false;"><i class="{{ $_style['actions_search'] }}"></i></a>
                            <a class="btn btn-secondary btn-sm" href="javascript:;" title="{{ ManagerTheme::getLexicon('reset') }}" onclick="resetSearch();return false;"><i class="{{ $_style['actions_refresh'] }}"></i></a>
                            <a class="btn btn-secondary btn-sm" href="javascript:;" title="{{ ManagerTheme::getLexicon('list_mode') }}" onclick="changeListMode();return false;"><i class="{{ $_style['actions_table'] }}"></i></a>
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="table-responsive">
                        <?php
                        $ds = $modx->getDatabase()->select("el.id, ELT(el.type , 'text-info " . $_style['actions_info'] . "', 'text-warning " . $_style['actions_triangle'] . "' , 'text-danger " . $_style['actions_cancel'] . "' ) as icon, el.createdon, el.source, el.eventid,IFNULL(wu.username,mu.username) as username", "{$tbl_event_log} AS el
