...
class DatePicker
{
    constructor(dp, options)
    {
        ...
        options = options || [];
        dp.options = {
            monthNames: (options.monthNames && options.monthNames.length === 12 ? options.monthNames : this.monthNames) || this.monthNames, daysInMonth: (options.daysInMonth && options.daysInMonth.length === 12 ? options.daysInMonth : this.daysInMonth) || this.daysInMonth, dayNames: (options.dayNames && options.dayNames.length === 7 ? options.dayNames : this.dayNames) || this.dayNames, startDay: options.startDay || this.startDay, dayChars: options.dayChars || this.dayChars, format: options.format || this.format, yearStart: options.yearStart || this.yearStart, yearRange: options.yearRange || this.yearRange, yearOrder: options.yearOrder || this.yearOrder, yearOffset: options.yearOffset || this.yearOffset,
        };
        dp.id = dp.name;
        dp.autocomplete = 'off';
    ...
    static getValue(dp)
    {
        if (dp.value !== '') {
            if (dp.options.format === 'dd-mm-YYYY hh:mm:00' || dp.options.format === 'dd-mm-YYYY') {
                let dateVals = dp.value.split(' ');
                let dateParts = dateVals[0].split('-');
    ...
    static updateValue(dp)
    {
        let el = document.querySelector('td.dp_selected');
        let ds;
        if (el) {
            ds = el.axis.split('|');
            let formatted = DatePicker.formatValue(dp, ds[0], ds[1], ds[2]);
            if (formatted !== '') {
                dp.value = formatted;
    ...
    static close(e)
    {
        ...
        let clickOutside = (e.target && e.target !== this.dp && e.target !== this.dp.container && !(DatePicker.hasChild(e.target, this.dp.container)));
        if (clickOutside) {
            if (this.dp.dirty) {
    ...
    static create(dp)
    {
        ...
        let firstDay = (1 - (7 + date.getDay() - dp.options.startDay) % 7);
        let monthSel = document.createElement('select');
        monthSel.id = dp.id + '_monthSelect';
        for (let m = 0; m < dp.options.monthNames.length; m++) {
        ...
        if (dp.options.yearOrder === 'desc') {
            for (y = dp.options.yearStart - dp.options.yearOffset; y > (dp.options.yearStart - dp.options.yearRange - 1); y--) {
        ...
        let timeTextBox = document.createElement('input');
        ...
        let calTableThead = document.createElement('thead');
        let calSelRow = document.createElement('tr');
        let calSelCell = document.createElement('th');
        ...
        let calTableTbody = document.createElement('tbody');
        let calDayNameRow = document.createElement('tr');
        let calDayNameCell;
        for (i = 0; i < dp.options.dayNames.length; i++) {
            calDayNameCell = document.createElement('th');
            calDayNameCell.innerHTML = dp.options.dayNames[(dp.options.startDay + i) % 7].substr(0, dp.options.dayChars);
        ...
        while (firstDay <= dp.options.daysInMonth[dp.month]) {
            ...
            for (i = 0; i < 7; i++) {
                ...
                if ((firstDay <= dp.options.daysInMonth[dp.month]) && (firstDay > 0)) {
                    ...
                    calDayCell.axis = dp.year + '|' + (parseInt(dp.month) + 1) + '|' + firstDay;
                ...
                if ((firstDay === parseInt(dp.oldDay)) && (dp.month === dp.oldMonth) && (dp.year === dp.oldYear)) {
                    calDayCell.classList.add('dp_selected');
                }
                if ((firstDay === dp.nowDay) && (parseInt(dp.month) === dp.nowMonth) && (parseInt(dp.year) === dp.nowYear)) {
        ...
        let calTimePara = document.createElement('p');
        ...
        dp.container.style.left = (dp.position.left + window.scrollX) + 'px';
        if (dp.position.top + (dp.container.offsetHeight) > window.innerHeight) {
            dp.container.style.top = (dp.position.top + window.scrollY) - dp.container.offsetHeight + 'px';
        } else {
            dp.container.style.top = (dp.position.top + dp.position.height + window.scrollY) + 'px';
        }
        let calDays = document.querySelectorAll('td.' + dp.id + '_calDay');
    ...
    static formatValue(dp, year, month, day)
    {
        let time = document.getElementById(dp.id + '_timeTextBox').value.split(':');
        if (!time[0] || time[0] === '' || time[0] < 0 || time[0] > 23) {
            dp.error = 'Invalid hours value: ' + time[0] + '\nAllowed range is 00:23';
            return '';
        }
        if (!time[1] || time[1] === '' || time[1] < 0 || time[1] > 59 || time[1].length !== 2) {
        ...
        let dateStr = dp.options.format.replace(/dd/i, day).replace(/mm/i, month).replace(/yyyy/i, year).replace(/hh/, time[0]).replace(/mm/, time[1]);
