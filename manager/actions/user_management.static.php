...
<form name="resource" method="post">
    ...
    <h1>
        <i class="<?= $_style['icon_user'] ?>"></i><?= $_lang['user_management_title'] ?><i
        ...
        <div class="container container-body">
            <div class="row searchbar form-group">
                <div class="col-sm-6 input-group">
                    <div class="input-group-btn">
                        <a class="btn btn-success btn-sm" href="index.php?a=11"><i
                                    class="<?= $_style['icon_add'] ?>"></i> <?= $_lang['new_user'] ?></a>
                    </div>
                </div>
                <div class="col-sm-6 ">
                    <div class="input-group float-right w-auto">
                        ...
                        <div class="input-group-append">
                            <a class="btn btn-secondary btn-sm" href="javascript:;" title="<?= $_lang["search"] ?>"
                               onclick="searchResource();return false;"><i
                                        class="<?= $_style['icon_search'] ?>"></i></a>
                            <a class="btn btn-secondary btn-sm" href="javascript:;" title="<?= $_lang["reset"] ?>"
                               onclick="resetSearch();return false;"><i class="<?= $_style['icon_refresh'] ?>"></i></a>
                            <a class="btn btn-secondary btn-sm" href="javascript:;" title="<?= $_lang["list_mode"] ?>"
                               onclick="changeListMode();return false;"><i class="<?= $_style['icon_table'] ?>"></i></a>
                        </div>
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="table-responsive">
                    <?php
                    $managerUsers = \EvolutionCMS\Models\ManagerUser::query()
                        ->select('manager_users.id', 'manager_users.username', 'user_roles.name as role', 'user_attributes.fullname', 'user_attributes.email', 'user_attributes.blocked', 'user_attributes.thislogin', 'user_attributes.logincount')
                        ->join('user_attributes', 'user_attributes.internalKey', '=', 'manager_users.id')
                        ->leftJoin('user_roles', 'user_attributes.role', '=', 'user_roles.id')
                        ->orderBy('user_attributes.blocked', 'ASC')->orderBy('user_attributes.thislogin', 'DESC');
                    $where = "";
