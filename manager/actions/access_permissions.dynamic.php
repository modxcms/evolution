...
$sql = 'SELECT * FROM '.$modx->getFullTableName('membergroup_names').' ORDER BY name';
$rs = mysql_query($sql);
if (mysql_num_rows($rs) < 1) {
...
// User Groups
	...
	$sql = 'SELECT '.
		...
		'FROM '.$modx->getFullTableName('membergroup_names').' AS groupnames '.
		'LEFT JOIN '.$modx->getFullTableName('member_groups').' AS groups ON groups.user_group = groupnames.id '.
		'LEFT JOIN '.$modx->getFullTableName('manager_users').' AS users ON users.id = groups.member '.
...
// User/Document Group Links
	...
	$sql = "SELECT ".
		...
		"FROM ".$modx->getFullTableName('membergroup_names')." AS groupnames ".
		"LEFT JOIN ".$modx->getFullTableName('membergroup_access')." AS groupacc ON groupacc.membergroup = groupnames.id ".
