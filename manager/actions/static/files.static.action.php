...
$editablefiles = array(".txt", ".php", ".shtml", ".html", ".htm", ".xml", ".js", ".css", ".etoCache", $friendly_url_suffix);
	...
	while ($file = $dir->read()) {
		if(!in_array($file, $excludes)) {
			...
			if(is_dir($newpath)) {
				...
				$files_array[$filecounter]['view'] = (in_array($type, $viewablefiles)) ? "<span style='cursor:pointer; width:20px;' onClick='viewfile(\"$webstart_path".substr($newpath, $len, strlen($newpath))."\");'><img src='media/images/icons/context_view.gif' border=0 align='absmiddle'></span>" : (($enablefiledownload && in_array($type, $uploadablefiles))? "<a href='$webstart_path".substr($newpath, $len, strlen($newpath))."' style='cursor:pointer; width:20px;'><img src='media/editor/images/ed_save.gif' title='".$_lang["file_download_file"]."' border=0 align='absmiddle'></a>":"<span class='disabledImage'><img src='media/images/icons/context_view.gif' border=0 align='absmiddle'></span>");
